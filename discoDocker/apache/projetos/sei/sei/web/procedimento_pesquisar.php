...
try {
  ...
  switch($_GET['acao']){
    ...
    case 'procedimento_pesquisar':
      ...
      if (!InfraString::isBolVazia($strPalavrasPesquisa) || count($arrNumIdSerie)|| count($arrNumIdUnidade)) {
...
if(0){?><style><?}
?>
  #divGeral {height:15em;width:99%;overflow:visible;}


  #lblPesquisa {positsion:absolute;left:0%;top:0%;width:50%;display:none;}
  #txtPesquisa {position:absolute;left:0%;top:0%;width:80%;}
  #ancAjuda {position:absolute;left:82%;top:0%;}
  #sbmPesquisar {position:absolute;left:87%;top:1%;}

  #lblSerie {position:absolute;left:0%;top:30%;width:50%;}
  #selSerie, .multipleSelectSerie {position:absolute;left:0%;top:42%;width:50%;}

  #lblUnidade {position:absolute;left:0%;top:60%;width:50%;}
  #selUnidade, .multipleSelectUnidade {position:absolute;left:0%;top:72%;width:50%;}
...
<form id="frmPesquisaProtocolo" name="frmPesquisaProtocolo" method="post" onsubmit="return OnSubmitForm();" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'].$strParametros)?>">
  ...
  <select multiple id="selSerie" name="selSerie[]" class="infraSelect multipleSelectSerie" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">
    <?=$strOptionsSeries;?>
  </select>

  <label id="lblUnidade" for="selUnidade" accesskey="" class="infraLabelOpcional">Unidade geradora:</label>
  <select multiple id="selUnidade" name="selUnidade[]" class="infraSelect multipleSelectUnidade" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">
      <?=$strOptionsUnidades;?>
  </select>
