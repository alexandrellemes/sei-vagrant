  ...
  class InfraArray {
    ...
    public static function ordenarArrayMultiplo(&$arr, $arrIndice, $arrTipoIndice, $arrTipoOrdenacao){
      ...
      if(count($arrTipoOrdenacao) != $numCountIndice){
        throw new InfraException('O número de critérios de ordenação deve corresponder ao número total de índices.');
      }
      ...
      for($i=0;$i<$numCount;$i++){
        for($j=0;$j<$numCountIndice;$j++){
          ...
          if ($strTipoIndice=='DTA'){
            ...
            $strValor =  $arrAux[2].str_pad($arrAux[1],2,'0',STR_PAD_LEFT).str_pad($arrAux[0],2,'0',STR_PAD_LEFT);
          }else if ($strTipoIndice=='DTH'){
            $arrAux = explode("/",$arr[$i][$strIndice]);
            $strValor =  substr($arrAux[2],0,4).str_pad($arrAux[1],2,'0',STR_PAD_LEFT).str_pad($arrAux[0],2,'0',STR_PAD_LEFT).str_replace(':','',substr($arrAux[2],5));
          }else{
        ...
        foreach($arrObjInfraDTO as $dto){
          ...
          if ($strValorAtributo!=''){
            if ($strPrefixo==InfraDTO::$PREFIXO_DTA){
              ...
              $strValorAtributo =  $arrAux[2].str_pad($arrAux[1],2,'0',STR_PAD_LEFT).str_pad($arrAux[0],2,'0',STR_PAD_LEFT);
            }else if ($strPrefixo==InfraDTO::$PREFIXO_DTH){
              $arrAux = explode("/",$strValorAtributo);
              $strValorAtributo =  substr($arrAux[2],0,4).str_pad($arrAux[1],2,'0',STR_PAD_LEFT).str_pad($arrAux[0],2,'0',STR_PAD_LEFT).str_replace(':','',substr($arrAux[2],5));
            }else if ($strPrefixo==InfraDTO::$PREFIXO_STR){
              $strValorAtributo = InfraString::transformarCaixaAlta(InfraString::excluirAcentos($strValorAtributo));
            }
          }

          if(!ctype_digit($strValorAtributo)){
     ...
     * @param String $strAtributo - nome do atributo (sem o tipo) existente nos objetos do array
     * @return Array o array sem repetições do atributo informado
     */
    public static function distinctArrInfraDTO($arrObjInfraDTO, $strAtributo){
      //CONTEM OS VALORES DE STRATRIBUTO JA ADICIONADOS
      ...
      foreach($arrObjInfraDTO as $dto){
        if (!InfraUtil::inArray($dto->get($strAtributo),$arrAdicionados)){
          $arrAdicionados[] = $dto->get($strAtributo);
          $ret[] = $dto;
        }
      }
      return $ret;
     ...
     */
    public static function converterArrInfraDTO($arrObjInfraDTO, $strAtributoValor=null, $varAtributoChave=null, $bolAtributoRepetido=false){
      $ret = array();
      ...
      if ($strAtributoValor==null){ //atributo null
        ...
        if ($dto!=null){
          return array(get_class($dto) => $ret);
        }
    ...
    */
    ...
    public static function mapearArrInfraDTO($arrObjInfraDTO, $strAtributoChave, $strAtributoValor, $bolTrimChave=false, $bolTrimValor=false){
      $ret = array();
      foreach($arrObjInfraDTO as $dto){
