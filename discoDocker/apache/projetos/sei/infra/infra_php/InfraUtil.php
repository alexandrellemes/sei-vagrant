...
class InfraUtil {
  ...
  public static function removerDir($dirname){
    if ($dirHandle = opendir($dirname)){
      ...
      while ($file = readdir($dirHandle)){
        ...
        if (is_dir($file)){
          if (!InfraUtil::removerDir($file)) return false;
        }else{
  ...
  public static function formatarDin($din,$dec=null){
    ...
    if (!InfraString::isBolVazia($din)){
      ...
      $sinal = '';
      if (substr($din,0,1)=='-'){//RETIRA O SINAL PARA AS OPERA«’ES E GUARDA PARA RECOLOCAR NO FINAL DA FUN«√O
        $sinal = '-';
        $din = substr($din,1);
      }
      $pos = strpos($din,'.');
      ...
      if ($dec==null){//$dec N√O INFORMADO
        ...
        if ($numTamDec==0){//PREENCHE COM ZEROS DO ⁄LTIMO N⁄MERO V¡LIDO AT… O LIMITE INFORMADO EM $dec
          for(;$dec>0;$dec--) $decimais = $decimais.'0';
  ...
  public static function validarEmail($email){
    ...
    */
    $regexp='/^\#([\w-]+(\.[\w-]+)*@(([A-Za-z\d][A-Za-z\d-]{0,61}[A-Za-z\d]\.)+[A-Za-z]{2,6}|\[\d{1,3}(\.\d{1,3}){3}\])|([^<^>^"]*)\<[\w-]+(\.[\w-]+)*@(([A-Za-z\d][A-Za-z\d-]{0,61}[A-Za-z\d]\.)+[A-Za-z]{2,6}|\[\d{1,3}(\.\d{1,3}){3}\])\>|((\s)*"[^"]*"(\s)*)\<[\w-]+(\.[\w-]+)*@(([A-Za-z\d][A-Za-z\d-]{0,61}[A-Za-z\d]\.)+[A-Za-z]{2,6}|\[\d{1,3}(\.\d{1,3}){3}\])\>)\#$/i';
  ...
  public static function verificarTempoProcessamento($inicio=false) {
    if(!$inicio) return microtime(true);
    $fim = microtime(true);
  ...
  public static function getStrMimeType($strNomeArquivo){
    ...
    switch($ext){
      ...
      case 'p7s': return 'application/octet-stream';
  ...
  */
  ...
  public static function montarNomeArquivoUpload($strUsuario, $numTimeStamp, $strNomeArquivo){
    return '['.InfraUtil::formatarNomeArquivo($strUsuario).']['.date('dmY-His',$numTimeStamp).']-'.InfraUtil::formatarNomeArquivo($strNomeArquivo);
  }
  ...
  public static function inArray($mixValorASerProcurado, $arrLista) {
    ...
    foreach($arrLista as $itemDaLista)
    {
      ...
      if ($strTipoDoValorProcurado!=$strTipoDoItemAtualDaLista)
      {
        ...
        if ((is_string($mixValorASerProcurado) && is_numeric($itemDaLista)) || (is_string($itemDaLista) && is_numeric($mixValorASerProcurado))){
   ...
   * @param $strTexto - texto para aplicaÁ„o do filtro
   * @return string
   */
  public static function filtrarISO88591($strTexto) {
    return preg_replace('/([\x00-\x08]|[\x0B-\x0C]|[\x0E-\x1F]|[\x7F-\x9F])/', '', str_replace(chr(0), '', $strTexto));
  }
  ...
  public static function verificarEnderecoPermitido($strEndereco, $arrPermitidos){
    foreach($arrPermitidos as $strPermitido){
      ...
      if (strlen($strPermitido)){
        if ($strPermitido=='*'){
          return true;
        }else if (($pos = strpos($strPermitido,'*'))!==false){
          ...
          if (strlen($strEndereco) >= (strlen($strPrefixoPermitido) + strlen($strSufixoPermitido))){
            ...
            $strPrefixoEndereco = ($strPrefixoPermitido == '') ? '' : substr($strEndereco, 0, strlen($strPrefixoPermitido));
            $strSufixoEndereco = ($strSufixoPermitido == '') ? '' : substr($strEndereco, strlen($strSufixoPermitido)*(-1));
        ...
        }else if ($strPermitido==$strEndereco){
          return true;
        }
      }
    }
  ...
  public static function validarMascara($strTexto, $strMascara, $bolParcial=false){
    ...
    if ($tamTexto>$tamMascara) return false;
    ...
    for ($i = 0; $i<$tamMascara && $i<$tamTexto; $i++) {
      switch ($strMascara[$i]) {
        case '#':
          if (preg_match('/[0-9]/', $strTexto[$i]) !== 1) return false;
          break;
        case 'A':
          if (preg_match('/[A-Z]/', $strTexto[$i]) !== 1) return false;
          break;
        case 'a':
          if (preg_match('/[a-z]/', $strTexto[$i]) !== 1) return false;
          break;
        case 'L':
          if (preg_match('/[A-Za-z]/', $strTexto[$i]) !== 1) return false;
          break;
        case 'H':
          if (preg_match('/[A-Za-z0-9]/', $strTexto[$i]) !== 1) return false;
          break;
        default:
          if ($strMascara[$i] != $strTexto[$i]) return false;
      }
    }
    return true;
  }
  ...
  public static function isBolLinhaDeComando(){
    ...
    if (empty($_SERVER['REMOTE_ADDR']) and !isset($_SERVER['HTTP_USER_AGENT']) and InfraArray::contar($_SERVER['argv']) > 0){
      return true;
    }
  ...
  public static function compararVersoes($strVersao1, $strOperador, $strVersao2, $numFator = 3){
    ...
    $tamVersao = count($arrV1);

    $numVersao1 = '';
    for($i=0;$i<$tamVersao;$i++){

      if (!is_numeric($arrV1[$i])){
        throw new InfraException('Vers„o '.$strVersao1.' possui caractere n„o numÈrico.');
      }

      if (strlen($arrV1[$i])>$numFator){
        throw new InfraException('Vers„o '.$strVersao1.' possui subconjunto maior que o fator de comparaÁ„o.');
      }

      $numVersao1 .= str_pad($arrV1[$i], $numFator, '0', STR_PAD_LEFT);
    }
    ...
    for($i=0;$i<$tamVersao;$i++){
      ...
      if (!is_numeric($arrV2[$i])){
        throw new InfraException('Vers„o '.$strVersao2.' possui caractere n„o numÈrico.');
      }

      if (strlen($arrV2[$i])>$numFator){
        throw new InfraException('Vers„o '.$strVersao2.' possui subconjunto maior que o fator de comparaÁ„o.');
      }

      $numVersao2 .= str_pad($arrV2[$i], $numFator, '0', STR_PAD_LEFT);
    }
  ...
  public static function validarUUID($strUUID){
    return preg_match('/^\{?[0-9a-f]{8}\-?[0-9a-f]{4}\-?[0-9a-f]{4}\-?[0-9a-f]{4}\-?[0-9a-f]{12}\}?$/i', $strUUID);
  }
