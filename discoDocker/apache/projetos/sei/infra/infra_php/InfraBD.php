          ...
          if ($atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL]!==null && $atributo[InfraDTO::$POS_ATRIBUTO_TAB_ORIGEM]===null){
            if (isset($arrPK[$key]) && $arrPK[$key] == InfraDTO::$TIPO_PK_SEQUENCIAL){
              ...
              if($bolOracle) {
                ...
                $item = $atributo[InfraDTO::$POS_ATRIBUTO_VALOR];
                if ($atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] == InfraDTO::$PREFIXO_BIN) {
                    $strNomeCampo = ":bin" . $key;
                } else {
                    $strNomeCampo = ":" . $key;
                }
                if ($item != 'NULL') {
              ...
              } else { //outros Bancos ou valor não STR
...
//      InfraDebug::getInstance()->gravar('TEMPO: '.$t.' s');
  ...
  public function alterar($objInfraDTO, $bolRetornarSql=false){
    try {
      ...
      */
      ...
      if ($numReg==0){
        throw new InfraException('Registro não encontrado em '.$objInfraDTO->getStrNomeTabela().'.',null,$sql);
      }
  ...
  public function desativar($objInfraDTO, $bolRetornarSql=false){
    try {
      ...
      if ($numReg==0){
        throw new InfraException('Registro de '.$objInfraDTO->getStrNomeTabela().' não encontrado para desativação.',null,$sql);
      }
  ...
  public function reativar($objInfraDTO, $bolRetornarSql=false){
    try {
      ...
      $numReg = $this->getObjInfraIBanco()->executarSql($sql);
      if ($numReg==0){
        throw new InfraException('Registro de '.$objInfraDTO->getStrNomeTabela().' não encontrado para reativação.',null,$sql);
      }
  ...
  public function contar($objInfraDTO, $bolRetornarSql=false) {
    try {
      ...
      if (!isset($rs[0]['total'])){
        throw new InfraException('Contagem não retornou nenhum valor.',null,$sql);
      }

      if (!is_numeric($rs[0]['total'])){
        throw new InfraException('Contagem não retornou valor numérico.',null,$sql);
      }
...
*/
  ...
  */
  ...
  protected function montarFromJoinWhere($objInfraDTO){
    ...
    //Se não possui relacionamentos opcionais OU é MUMPS (que não aceita sintaxe INNER JOIN e LEFT JOIN)
