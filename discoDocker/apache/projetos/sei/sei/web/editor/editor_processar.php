...
function  inicializar(){
  ...
  CKEDITOR.on('instanceReady', function( evt ) {
    ...
    evt.editor.on( 'beforepaste', function( evt ) {
      ...
      ev.data.dataValue = ev.data.dataValue.replace(/<b\b[^>]*>/gi, "<b>");
      ...
      ev.data.dataValue = ev.data.dataValue.replace(/(<table[^<>]*)(align="[^<>]*?")([^<>]*>)/gi, "$1$3");
    ...
    if (ifr) {
      ...
      if (objArvore && objArvore.getNo(<?=$_GET['id_procedimento']?>) != null) {
        ...
        if (objNo && objNo.id =='<?=$_GET['id_documento']?>') {
          if (bolFechar){
            ifr.src = '<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao=procedimento_visualizar&acao_origem=' . $_GET['acao'] . $strParametros . '&montar_visualizacao=1')?>';
          } else {
            window.opener.document.getElementById('ifrArvoreHtml').contentWindow.location.reload();
          }
        }
      }
    }
  }
  if (bolFechar) window.close();
}
