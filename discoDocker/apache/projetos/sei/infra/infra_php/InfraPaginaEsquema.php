...
abstract class InfraPaginaEsquema extends InfraPagina {
	...
	public function getStrTextoBarraTribunal(){return null;}
	...
	public function getStrEsquemaCores(){
		...
		if ($this->strEsquemaCores == null){
			...
			if ($this->getObjInfraSessao()!=null && $this->getObjInfraSessao()->isBolHabilitada() && $this->getObjInfraSessao()->getObjInfraIBanco()!=null){
...
<link href="'.$strDiretorioCssGlobal.'/'.$this->getArquivoCssGlobalN().'?'.$numVersao.'" rel="stylesheet" type="text/css" media="all" />'."\n";

		if ( $this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA && $this->getStrMenuSistema() != null){
			if ($this->obterTipoMenu() == self::$MENU_NORMAL) {
				echo '<link href="' . $strDiretorioCssGlobal . '/' . $this->getArquivoCssMenuGlobal() . '?' . $this->getNumVersao() . '" rel="stylesheet" type="text/css" media="all" />' . "\n";
			}else if ($this->adicionarJQuery()){
				echo '<link href="' . $strDiretorioCssGlobal . '/smartmenu/sm-core-css.css?' . $this->getNumVersao() . '" rel="stylesheet" type="text/css" media="all" />' ."\n".
						'<link href="' . $strDiretorioCssGlobal . '/smartmenu/sm-'.$this->obterSmartMenuClass().'/sm-'.$this->obterSmartMenuClass().'.css?' . $this->getNumVersao() . '" rel = "stylesheet" type = "text/css" media="all" />'."\n";
			}
		}

echo '<link href="'.$this->getDiretorioEsquemas().'/'.$this->getStrEsquemaCores().'/'.$this->getArquivoCssEsquema().'?'.$numVersao.'" rel="stylesheet" type="text/css" media="all" />
<link href="'.$this->getDiretorioEsquemas().'/'.$this->getStrEsquemaCores().'/'.$this->getArquivoCssEsquemaN().'?'.$numVersao.'" rel="stylesheet" type="text/css" media="all" />
...
<link href="'.$this->getDiretorioCssLocal().'/infra-esquema-local.css?'.$numVersao.'" rel="stylesheet" type="text/css" media="all" />'."\n";
...
';
	...
	public function abrirBody($strLocalizacao='',$strAtributos=''){
		...
		if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA && $this->getStrMenuSistema() != null){
			$strAtributos = $this->complementarAtributo($strAtributos,'onload','infraMenuSistemaEsquema(true, null, \''.$this->getDiretorioImagensGlobal().'\');');
		}
		...
		if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA || $this->getTipoPagina()==self::$TIPO_PAGINA_SEM_MENU){
			...
			if ($strTextoBarraTribunal==null){
				...
				switch($this->getNumIdOrgaoBarraTribunal()){
					case 1:
						echo '<a href="http://www.trf4.jus.br" target="_blank" title="Site do Tribunal Regional Federal da 4ª Região" tabindex="'.$this->getProxTabBarraTribunal().'">Tribunal Regional Federal da 4ª Região</a>'."\n";
						break;

					case 2:
						echo '<a href="http://www.jfrs.jus.br" target="_blank" title="Site da Justiça Federal do Rio Grande do Sul" tabindex="'.$this->getProxTabBarraTribunal().'">Seção Judiciária do Rio Grande do Sul</a>'."\n";
						break;

					case 3:
						echo '<a href="http://www.jfsc.jus.br" target="_blank" title="Site da Justiça Federal de Santa Catarina" tabindex="'.$this->getProxTabBarraTribunal().'">Seção Judiciária de Santa Catarina</a>'."\n";
						break;

					case 4:
						echo '<a href="http://www.jfpr.jus.br" target="_blank" title="Site da Justiça Federal do Paraná" tabindex="'.$this->getProxTabBarraTribunal().'">Seção Judiciária do Paraná</a>'."\n";
						break;

					default:
						if ($this->getObjInfraSessao()!=null){
							echo '<label>'.self::tratarHTML($this->getObjInfraSessao()->getStrDescricaoOrgaoSistema()).'</label>'."\n";
						}
				}
			}else{
				echo ($this->isBolTratarHtmlBarraTribunal() ? self::tratarHTML($strTextoBarraTribunal) : $strTextoBarraTribunal);
			}
	...
	private function montarBarraSistema() {
		...
		if ( ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA && $this->getStrMenuSistema() != null) || $this->getTipoPagina()==self::$TIPO_PAGINA_SEM_MENU){

			echo '<div id="divInfraBarraSistema" class="infraBarraSistema" style="height:4em;background: url('.$this->getDiretorioEsquemas().'/'.$this->getStrEsquemaCores().'/bg_barra_sistema_conteudo.png'.');">'."\n".
			...
			echo '</div>'."\n".
					...
					'<img src="'.$this->getDiretorioEsquemas().'/'.$this->getStrEsquemaCores().'/bg_barra_sistema_curva.png" />'.
					'</div>';
		}
	}
	...
	public function montarPaginaConfiguracao(){
		PaginaInfra::setObjInfraPagina($this);
		SessaoInfra::setObjInfraSessao($this->getObjInfraSessao());
	...
	public function montarPaginaErro($strErro, $strDetalhes, $strTrace){
		...
		$this->fecharHead();
		$this->abrirBody('Erro','onload="parent.infraOcultarAviso();"');
		...
		if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA || $this->getTipoPagina()==self::$TIPO_PAGINA_SEM_MENU){
			...
			$arrComandos[] = $this->montarBotaoFecharExcecao();
		}
    ...
    if ($strIcone==null){
      $strIcone = $this->getDiretorioImagensGlobal().'/configuracao.gif';
		...
		return '<a id="lnkConfiguracaoSistema" href="'.$strLink.'" tabindex="'.$this->getProxTabBarraSistema().'"><img src="'.$strIcone.'" title="Configurações do Sistema" alt="Configurações do Sistema" class="infraImg" /></a>';
	}

	public function montarLinkMenu(){
		$strLink = '<a id="lnkInfraMenuSistema" onclick="infraMenuSistemaEsquema(false, null, \''.$this->getDiretorioImagensGlobal().'\');" tabindex="'.$this->getProxTabBarraSistema().'">';
		$strLink .= '<img id="imgInfraMenuSistema"';
		if ($this->getStrCookieMenuMostrar()=='N'){
			$strLink .= ' src="'.$this->getDiretorioImagensGlobal().'/botao_menu_abrir.gif" title="Exibir Menu do Sistema" alt="Exibir Menu do Sistema"';
		}else{
			$strLink .= ' src="'.$this->getDiretorioImagensGlobal().'/botao_menu_fechar.gif" title="Ocultar Menu do Sistema" alt="Ocultar Menu do Sistema"';
		}
	...
	public function montarSelectUnidades(){
      ...
      $objInfraSessao->setArrParametrosRepasseLink($arrParametrosRepasseLink);
		...
		return '<span id="spanInfraUnidade" class="infraUnidade">'.$str.'</span>&nbsp;';
	}
}
?>