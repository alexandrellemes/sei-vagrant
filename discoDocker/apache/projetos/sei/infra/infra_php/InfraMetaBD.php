...
class InfraMetaBD {
  ...
  private function validarIdentificador($strIdentificador){
    ...
    if ($this->bolValidarIdentificador && strlen($strIdentificador) > 30){
      throw new InfraException('Identificador ['.$strIdentificador.'] possui tamanho superior a 30 caracteres.');
    }
  ...
  public function obterNomeColunasConstraint($strNomeTabela, $strNomeConstraint, $strTipoConstraint){
    ...
    if ($this->objInfraIBanco instanceof InfraSqlServer){
      ...
      $sql = 'SELECT DISTINCT lower(table_name) as table_name, lower(constraint_name) as constraint_name, lower(column_name) as column_name FROM INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE'.
             ' where lower(table_name)=\''.strtolower($strNomeTabela).'\'';
      ...
      $sql = 'select DISTINCT lower(table_name) as table_name, lower(constraint_name) as constraint_name, lower(column_name) as column_name'.
  ...
  public function obterIndices($arrTabelasIgnorar = null, $strNomeTabela = null){
    $ret = null;
  ...
  public function alterarColuna($strTabela, $strColuna, $strTipo, $strNull){
    ...
    $sql .= ' '.$this->validarIdentificador($strColuna).' '.$strTipo.' '.$this->getOptionNull($strTabela, $strColuna,$strNull);
  ...
  public function adicionarChavePrimaria($strTabela, $strNomePK, $arrCampos){
    $this->objInfraIBanco->executarSql('alter table '.$strTabela.' add constraint '.$this->validarIdentificador($strNomePK).' primary key ('.implode(',',$arrCampos).')');
  }
  ...
  public function adicionarChaveEstrangeira($strNomeFK,$strTabela,$arrCampos,$strTabelaOrigem, $arrCamposOrigem, $bolCriarIndice = true){
    ...
    $this->objInfraIBanco->executarSql('alter table '.$strTabela.' add constraint '.$this->validarIdentificador($strNomeFK).' foreign key ('.implode(',',$arrCampos).') references '.$strTabelaOrigem.' ('.implode(',',$arrCamposOrigem).')');
  }
  ...
  public function criarIndice($strTabela, $strIndex, $arrColunas, $bolUnique = false){
    ...
    if (isset($arrIndices[$strTabela])){
      ...
      if (!$bolEncontrou) {
        ...
        foreach ($arrIndicesBanco as $strNomeIndiceBanco) {
          ...
          $arrColunasIndiceBanco = $this->prepararColunasComparacao($arrIndices[$strTabela][$strNomeIndiceBanco]);

          if ($arrColunasIndiceNovo == $arrColunasIndiceBanco) {

            $this->renomearIndice($strTabela, $strNomeIndiceBanco, $strIndex);
    ...
    if (!$bolEncontrou) {
      $this->objInfraIBanco->executarSql('create '.($bolUnique ? 'unique' : '').' index '.$this->validarIdentificador($strIndex).' on '.$strTabela.' ('.implode(',', $arrColunas).')');
    }
  }
  ...
  public function renomearIndice($strTabela, $strIndexAntigo, $strIndexNovo){
    $sql = '';
    if ($this->objInfraIBanco instanceof InfraMySql){
      $sql = 'alter table '.$strTabela.' rename index '.$strIndexAntigo.' to '.$strIndexNovo;
    }else if ($this->objInfraIBanco instanceof InfraSqlServer){
      $sql = 'EXEC sp_rename \''.$strTabela.'.'.$strIndexAntigo.'\', \''.$strIndexNovo.'\', \'INDEX\'';
    }else if ($this->objInfraIBanco instanceof InfraOracle){
      $sql = 'alter index '.$strIndexAntigo.' rename to '.$strIndexNovo;
    }
    $this->objInfraIBanco->executarSql($sql);
  }
  ...
  public function processarIndicesChavesEstrangeiras($arrTabelas = null, $arrChavesEstrangeirasIgnorar = null){
    ...
    foreach($arrConstraints as $arrConstraint) {
      ...
      if ($arrTabelas == null || in_array($strNomeTabela, $arrTabelas)) {
        ...
        if ($arrConstraint['constraint_type'] == 'primary key') {
          ...
          $arrColunasPKDetalhes = $this->obterNomeColunasConstraint($arrConstraint['table_name'], $arrConstraint['constraint_name'], $arrConstraint['constraint_type']);

          $arrColunasPK = array();
          foreach ($arrColunasPKDetalhes as $arrColunasPKDetalhe) {
            $arrColunasPK[] = $arrColunasPKDetalhe['column_name'];
          }
          ...
          if ($arrChavesEstrangeirasIgnorar == null || !in_array($strNomeIndiceFK, $arrChavesEstrangeirasIgnorar)) {

            $arrColunasFKDetalhes = $this->obterNomeColunasConstraint($arrConstraint['table_name'], $arrConstraint['constraint_name'], $arrConstraint['constraint_type']);

            $arrColunasFK = array();
            foreach ($arrColunasFKDetalhes as $arrColunasFKDetalhe) {
              $arrColunasFK[] = $arrColunasFKDetalhe['column_name'];
            }
    ...
    foreach($arrFKs as $strNomeTabela => $arrFKsTabela){
      ...
      if (InfraDebug::isBolProcessar()) {
        InfraDebug::getInstance()->gravarInfra('[InfraMetaBD->processarIndicesChavesEstrangeiras]: '.$strNomeTabela);
      }
      ...
      foreach($arrFKsTabela as $strNomeIndiceFK => $arrColunasFK){
        ...
        if (InfraDebug::isBolProcessar()) {
          InfraDebug::getInstance()->gravarInfra('[InfraMetaBD->processarIndicesChavesEstrangeiras]: --> '. $strNomeIndiceFK);
        }
