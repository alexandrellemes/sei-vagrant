...
class InfraString {
	...
	private static $arrMinusculas               = array('a',        'a',        'a',        'a',     'a',       'e',        'e',        'e',      'e',       'i',        'i',      'o',        'o',       'o',       'o',      'u',        'u',      'c',        'n');
	private static $arrMaiusculas               = array('A',        'A',        'A',        'A',     'A',       'E',        'E',        'E',      'E',       'I',        'I',      'O',        'O',       'O',       'O',      'U',        'U',      'C',        'N');
	private static $arrMinusculasAcentuadas     = array('á',        'ã',        'à',        'ä',     'â',       'é',        'è',        'ë',      'ê',       'í',        'ï',      'ó',        'õ',       'ô',       'ö',      'ú',        'ü',      'ç',        'ñ');
	private static $arrMaiusculasAcentuadas     = array('Á',        'Ã',        'À',        'Ä',     'Â',       'É',        'È',        'Ë',      'Ê',       'Í',        'Ï',      'Ó',        'Õ',       'Ô',       'Ö',      'Ú' ,       'Ü',      'Ç',        'Ñ');
	private static $arrMinusculasAcentuadasHTML = array('&aacute;', '&atilde;', '&agrave;', '&auml;','&acirc;', '&eacute;', '&egrave;', '&euml;', '&ecirc;', '&iacute;', '&iuml;', '&oacute;', '&otilde;','&ocirc;', '&ouml;', '&uacute;', '&uuml;', '&ccedil;', '&ntilde;');
	private static $arrMaiusculasAcentuadasHTML = array('&Aacute;', '&Atilde;', '&Agrave;', '&Auml;','&Acirc;', '&Eacute;', '&Egrave;', '&Euml;', '&Ecirc;', '&Iacute;', '&Iuml;', '&Oacute;', '&Otilde;','&Ocirc;', '&Ouml;', '&Uacute;', '&Uuml;', '&Ccedil;', '&Ntilde;');
...
/** 
  ...
  */ 	
  ...
  public static function prepararIndexacao($txt, $bolSomenteLetrasNumeros = true) {
		$ret = '';
		$txt = trim($txt);
		if ( $txt != '' ) {

		  $ret = self::excluirAcentos($txt);
      $ret = strtolower($ret);
  ...
  public static function prepararPesquisaDTO(InfraDTO $objDto, $strAtributo, $strAtributoPesquisa = null, $bolUnsetAtributo = true) {
    if ($objDto->isSetAtributo($strAtributo)){
      if (!InfraString::isBolVazia($objDto->get($strAtributo))) {
        $strPalavrasPesquisa = InfraString::prepararPesquisa($objDto->get($strAtributo));
        ...
        if ($numPalavrasPesquisa){
          ...
          if ($numPalavrasPesquisa==1){
            if($strAtributoPesquisa == null) {
              ...
              $objDto->set($strAtributoPesquisa, $arrPalavrasPesquisa[0], InfraDTO::$OPER_LIKE);
            }
          }else{
            if($strAtributoPesquisa == null) {
              $a = array_fill(0, $numPalavrasPesquisa, $strAtributo);
            }else{
              $a = array_fill(0,$numPalavrasPesquisa,$strAtributoPesquisa);
            }
 ...
 */
 ...
 public static function formatarNome($strNome){
  ...
  private static function contarCaracteresTrechosDestacados($arrPosDestaque, $lengthReticenciasIni,$textoDestaque,$lengthReticencias){
    $lengthJaComputado = 0;
  ...
  public static function abreviarDestacarTexto($texto, $substring, $tamanhoMax, $htmlDestaqueIni, $htmlDestaqueFim, $maxTrechos = 3){
    ...
    if (strlen($texto) <= $tamanhoMax){
      ...
      do{
        ...
        $lengthJaComputado = self::contarCaracteresTrechosDestacados($arrPosDestaque, $lengthReticenciasIni,$textoDestaque,$lengthReticencias);
      ...
      for ($i=0;$i<$numSubstr;$i++){
        $textoDestaque .= strip_tags(substr($textoDestaqueTemp, $cursor, $arrPosDestaque[$i]['ini'] - $cursor)) . substr($textoDestaqueTemp, $arrPosDestaque[$i]['ini'], $arrPosDestaque[$i]['fim'] - $arrPosDestaque[$i]['ini']) ;
        $cursor = $arrPosDestaque[$i]['fim'];
      }
      $textoDestaque .= strip_tags(substr($textoDestaqueTemp, $arrPosDestaque[$i-1]['fim'], strlen($textoDestaqueTemp) - $arrPosDestaque[$i-1]['fim'])) ;
      ...
      do {
        ...
        for ($i = 0; $i < $numSubstr; $i++) {
          ...
          if ($arrPosDestaqueTemp[$i]['fim'] < strlen($textoDestaque) && ($arrPosDestaqueTemp[$i]['fim'] + $tamanhoFrag) > strlen($textoDestaque)) {
