...
//LEITURA		
		...
		public function formatarPesquisaStr($strTabela,$strCampo,$strValor){
		  switch ($this->driver) {
    		  case 'mysql':
    		    return 'upper('.$strCampo.') LIKE \''.str_replace('\'','\'\'',InfraString::transformarCaixaAlta($strValor)).'\' ';
    		  case 'freetds':
    ...
    public function abrirTransacao(){
      ...
      if (strpos(strtoupper($select),'DISTINCT')===false){
        ...
        for($i=0;$i<count($arrOrder);$i++){
          $order .= ' ';
          if (strtoupper($arrOrder[$i])=='ORDER' || strtoupper($arrOrder[$i])=='BY' || strtoupper($arrOrder[$i])==','){
            $order .= $arrOrder[$i];
          }else{
      ...
      if (InfraDebug::isBolProcessar()) {
        $numSeg = InfraUtil::verificarTempoProcessamento($numSeg);
        InfraDebug::getInstance()->gravarInfra('[InfraODBC->executar] ' . $arrInfo[self::$ODBC_INFO_ROWS_MATCHED] . ' registro(s) afetado(s)');
