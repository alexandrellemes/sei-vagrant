    ...
    case 'acompanhamento_cadastrar':
      ...
      if (PaginaSEI::getInstance()->isBolArvore() && !isset($_POST['hdnIdProcedimento'])){
        ...
        if (isset($_GET['id_acompanhamento'])){
          $dto->setNumIdAcompanhamento($_GET['id_acompanhamento']);
        }else{
          $dto->setDblIdProtocolo($numIdProcedimento);
        }

        $dto->setOrdNumIdAcompanhamento(InfraDTO::$TIPO_ORDENACAO_DESC);

        $objAcompanhamentoRN = new AcompanhamentoRN();
        $arr = $objAcompanhamentoRN->listar($dto);
        ...
        if (count($arr)){
          ...
          if (count($arr) > 1){
            PaginaSEI::getInstance()->setStrMensagem('Existe mais de um registro de Acompanhamento Especial para este processo na unidade.\n\nO registro mais recente foi selecionado para exibição.', InfraPagina::$TIPO_MSG_AVISO);
          }
