  ...
  switch($_GET['acao']){
    case 'protocolo_modelo_cadastrar':
      ...
      if (PaginaSEI::getInstance()->isBolArvore() && !isset($_POST['hdnIdProtocolo'])){
        ...
        $dto->setOrdDblIdProtocoloModelo(InfraDTO::$TIPO_ORDENACAO_DESC);
      
        $objProtocoloModeloRN = new ProtocoloModeloRN();
        $arr = $objProtocoloModeloRN->listar($dto);
        ...
        if (count($arr)){
          ...
          if (count($arr) > 1){
            PaginaSEI::getInstance()->setStrMensagem('Existe mais de um registro de Modelo Favorito para este documento na unidade.\n\nO registro mais recente foi selecionado para exibição.', InfraPagina::$TIPO_MSG_AVISO);
          }
