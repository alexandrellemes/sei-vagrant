...
class EstatisticasRN extends InfraRN {
  ...
  //unidade hoje
  public static $TIPO_UNIDADE_HOJE_PROCESSOS = 19;
  public static $TIPO_UNIDADE_HOJE_DOCUMENTOS_UNIDADE_NAO_ASSINADOS = 20;
  public static $TIPO_UNIDADE_HOJE_DOCUMENTOS_UNIDADE_ASSINADOS = 21;
  public static $TIPO_UNIDADE_HOJE_DOCUMENTOS_BLOCO_NAO_ASSINADOS = 22;
  public static $TIPO_UNIDADE_HOJE_DOCUMENTOS_BLOCO_ASSINADOS = 23;
  public static $TIPO_UNIDADE_HOJE_USUARIO_ATRIBUICAO = 24;
  ...
  public static $TITULO_UNIDADE_HOJE_DOCUMENTOS_UNIDADE_NAO_ASSINADOS 		= 'Documentos da Unidade não Assinados';
  ...
  public static $TITULO_UNIDADE_HOJE_DOCUMENTOS_BLOCO_NAO_ASSINADOS 		= 'Documentos em Bloco não Assinados';
  public static $TITULO_UNIDADE_HOJE_DOCUMENTOS_BLOCO_ASSINADOS 		= 'Documentos em Bloco Assinados';
  public static $TITULO_UNIDADE_HOJE_USUARIO_ATRIBUICAO 		= 'Atribuição de Processos';
  ...
  protected function gerarOuvidoriaConectado(EstatisticasDTO $parObjEstatisticasDTO) {
    ...
    if (count($parObjEstatisticasDTO->getArrObjUnidadeDTO())==0){
      $objInfraException->lancarValidacao('Nenhuma unidade de ouvidoria encontrada.');
    }
     
    if (count($parObjEstatisticasDTO->getArrObjTipoProcedimentoDTO())==0){
        ...
        $dtoBase->setDblQuantidade(null);
            ...
            $this->acumular($dto);
					...
					if (isset($arrAndamentos[$objEstatisticaAtividadeDTO->getDblIdProtocolo()])){
						...
						$numAndamentos = count($arrAndamentosProtocolo);
      ...
      */
      ...
      if (count($arrObjEstatisticasAtividadeDTO)){
	      ...
	      foreach($arrAndamentosPorProtocolo as $dblIdProtocolo => $arrAndamentosProtocolo){
	        ...
	        $numAndamentosProtocolo = count($arrAndamentosProtocolo);
	        ...
	        foreach($arrAndamentosPorUnidade as $numIdUnidade => $arrAndamentos){
	          ...
	          $objEstatisticasAtividadeDTOUltimo = $arrAndamentos[count($arrAndamentos)-1];
            ...
            $numAndamentos = count($arrAndamentos);
            ...
            $numTarefas = count($arrTarefaPeriodo);
          }
	      ...
	      foreach($arrDesempenhoPorUnidade as $numIdTipoProcedimento => $arrTemp){
  	      foreach($arrTemp as $numIdUnidade => $arr){
  	        ...
  	        $numTempoTipoUnidade = bcdiv($arr[0],count($arr[1]),0);
  	        ...
  	        $dto->setDthSnapshot($dthSnapshot);
  	        $dto->setDblQuantidade(count($arr[1]));
  	        $this->acumular($dto);
  	        
  	        if (!isset($arrTempoPorUnidade[$numIdUnidade])){
  	          $arrTempoPorUnidade[$numIdUnidade][0] = $arr[0];
  	          $arrTempoPorUnidade[$numIdUnidade][1] = count($arr[1]);
  	        }else{
  	          $arrTempoPorUnidade[$numIdUnidade][0] = $arrTempoPorUnidade[$numIdUnidade][0] + $arr[0];
  	          $arrTempoPorUnidade[$numIdUnidade][1] = $arrTempoPorUnidade[$numIdUnidade][1] + count($arr[1]);
  	        }
  	      }
	      }
