...
abstract class InfraPagina {
  ...
  protected static $TAB_FIM_MENU = 450;
  ...
  protected static $POS_SES_ULTIMO_ACESSO = 16;

  private static $POS_SES_MSG_TIPO = 0;
  private static $POS_SES_MSG_CONTEUDO = 1;
  ...
  public function isBolIphone() {
    return (strpos($_SERVER['HTTP_USER_AGENT'], 'iPhone')!==false && strpos($_SERVER['HTTP_USER_AGENT'], 'Trident/')==false);
  }
  ...
  public static function getNumVersaoFirefox($strUserAgent = null) {
    ...
    if ($posNavegador!==false) {
      ...
      if ($posFimVersao!==false) {
        $strVersao = substr($strUserAgent, $posNavegador + strlen('Firefox/'), $posFimVersao - ($posNavegador + strlen('Firefox/')));
      } else {
  ...
  public static function getNumVersaoChrome($strUserAgent = null) {
    ...
    if ($posNavegador!==false) {
      $posFimVersao = strpos($strUserAgent, ' ', $posNavegador);
      if ($posFimVersao!==false) {
        $strVersao = substr($strUserAgent, $posNavegador + strlen('Chrome/'), $posFimVersao - ($posNavegador + strlen('Chrome/')));
      }
    }
    return self::formatarVersaoNavegador($strVersao);
  }
  ...
  public static function getNumVersaoInternetExplorer($strUserAgent = null) {
    ...
    if ($posNavegador!==false) {
      $posFimVersao = strpos($strUserAgent, ';', $posNavegador);
      if ($posFimVersao!==false) {
        $strVersao = substr($strUserAgent, $posNavegador + strlen('MSIE '), $posFimVersao - ($posNavegador + strlen('MSIE ')));
      }
      ...
      if ($posFimVersao1===false) $posFimVersao1 = $numTamUA;
      if ($posFimVersao2===false) $posFimVersao2 = $numTamUA;
      if ($posFimVersao3===false) $posFimVersao3 = $numTamUA;

      $posFimVersao = $numTamUA;
      if ($posFimVersao1<$posFimVersao2 && $posFimVersao1<$posFimVersao3) {
        $posFimVersao = $posFimVersao1;
      } else if ($posFimVersao2<$posFimVersao1 && $posFimVersao2<$posFimVersao3) {
        $posFimVersao = $posFimVersao2;
      } else if ($posFimVersao3<$posFimVersao1 && $posFimVersao3<$posFimVersao2) {
  ...
  public static function getNumVersaoSafariIpad($strUserAgent = null) {
    ...
    if (strpos($strUserAgent, 'iPad;')!==false) {
      ...
      if ($posNavegador!==false) {
        $posFimVersao = strpos($strUserAgent, ' ', $posNavegador);
        if ($posFimVersao!==false) {
          $strVersao = substr($strUserAgent, $posNavegador + strlen('Version/'), $posFimVersao - ($posNavegador + strlen('Version/')));
        }
      }
    }
    return self::formatarVersaoNavegador($strVersao);
  ...
  public static function getNumVersaoSafari($strUserAgent = null) {
    ...
    if (strpos($strUserAgent, 'iPad;')===false && strpos($strUserAgent, 'Chrome/')===false && strpos($strUserAgent, 'Safari/')!==false) {
      $posNavegador = strpos($strUserAgent, 'Version/');
      if ($posNavegador!==false) {
  ...
  public function montarStyle() {
    ...
    echo '<link href="' . $strDiretorioCssGlobal . '/' . $this->getArquivoCssGlobal() . '?' . $numVersao . '" rel="stylesheet" type="text/css" media="all" />' . "\n" .
        ...
        '<link href="' . $this->getDiretorioCssLocal() . '/infra-local.css?' . $numVersao . '" rel="stylesheet" type="text/css" media="all" />' . "\n";
    ...
    echo '<link href="' . $strDiretorioCssGlobal . '/infra-ajax.css?' . $numVersao . '" rel="stylesheet" type="text/css" media="all" />' . "\n" .
        '<link href="' . $strDiretorioJavascriptGlobal . '/calendario/infra-calendario.css?' . $numVersao . '" rel="stylesheet" type="text/css" media="all" />' . "\n" .
    ...
    if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA && $this->getStrMenuSistema()!=null) {
      if ($this->obterTipoMenu()==self::$MENU_NORMAL) {
        echo '<link href="' . $strDiretorioCssGlobal . '/' . $this->getArquivoCssMenuGlobal() . '?' . $numVersao . '" rel="stylesheet" type="text/css" media="all" />' . "\n" .
            '<link href="' . $this->getDiretorioCssLocal() . '/menu-local.css?' . $numVersao . '" rel="stylesheet" type="text/css" media="all" />' . "\n";
      } else if ($this->adicionarJQuery()) {
        echo '<link href="' . $strDiretorioCssGlobal . '/smartmenu/sm-core-css.css?' . $numVersao . '" rel="stylesheet" type="text/css" media="all" />' . "\n" .
            '<link href="' . $strDiretorioCssGlobal . '/smartmenu/sm-' . $this->obterSmartMenuClass() . '/sm-' . $this->obterSmartMenuClass() . '.css" rel = "stylesheet" type = "text/css" media="all" />' . "\n";
      }
    }
    ...
    } else if ($this->getNumTipoBrowser()==self::$TIPO_BROWSER_IE7) {
    ...
    if ($this->adicionarJQuery()) {
      echo '<link href="' . $strDiretorioJavascriptGlobal . '/jquery/jquery-ui-1.11.1/jquery-ui.min.css?1.11.1" rel="stylesheet" type="text/css" media="all" />' . "\n" .
          ...
          '<link href="' . $strDiretorioJavascriptGlobal . '/multiple-select/multiple-select.css?' . $numVersao . '" rel="stylesheet" type="text/css" media="all" />' . "\n";
    }
  }
  ...
  public function adicionarStyle($strHref, $strMedia = 'all') {
    echo '<link href="' . $strHref . '?' . $this->getNumVersao() . '" rel="stylesheet" type="text/css" media="' . $strMedia . '" />' . "\n";
  }

  public function adicionarJavaScript($strSrc) {
    echo '<script type="text/javascript" charset="iso-8859-1" src="' . $strSrc . '?' . $this->getNumVersao() . '"></script>' . "\n";
  }

  public function montarConfigGlobal() {
    echo '<script type="text/javascript">var INFRA_PATH_IMAGENS=\''.$this->getDiretorioImagensGlobal().'\',INFRA_PATH_JS=\''.$this->getDiretorioJavaScriptGlobal().'\',INFRA_PATH_CSS=\''.$this->getDiretorioCssGlobal().'\';</script>'."\n";
  }
  ...
  public function montarJavaScript() {
    ...
    $strDiretorioJavascriptGlobal = $this->getDiretorioJavaScriptGlobal();
    $numVersao = $this->getNumVersao();
    $this->montarConfigGlobal();
    if ($this->adicionarJQuery()) {
      echo '
<script type="text/javascript" charset="utf-8" src="' . $strDiretorioJavascriptGlobal . '/jquery/jquery-1.12.4.min.js?1.12.4"></script>
<script type="text/javascript" charset="utf-8" src="' . $strDiretorioJavascriptGlobal . '/jquery/jquery-ui-1.11.1/jquery-ui.min.js?1.11.1"></script>
<script type="text/javascript" charset="utf-8" src="' . $strDiretorioJavascriptGlobal . '/multiple-select/jquery.multiple.select.js?' . $numVersao . '"></script>
...
';
      ...
      if ($this->obterTipoMenu()==self::$MENU_SMART) {
        echo '
<script type="text/javascript" charset="utf-8" src="' . $strDiretorioJavascriptGlobal . '/smartmenu/jquery.smartmenus.js"></script>
<script type="text/javascript" charset="utf-8" src="' . $strDiretorioJavascriptGlobal . '/smartmenu/addons/keyboard/jquery.smartmenus.keyboard.js"></script>
    ...
    if ($this->adicionarD3()) {
      echo '
<script type="text/javascript" charset="utf-8" src="' . $strDiretorioJavascriptGlobal . '/D3/d3.v3.min.js"></script>
...
<script type="text/javascript" charset="iso-8859-1" src="' . $strDiretorioJavascriptGlobal . '/InfraMenu.js?' . $numVersao . '"></script>
<script type="text/javascript" charset="iso-8859-1" src="' . $strDiretorioJavascriptGlobal . '/InfraBotaoMenu.js?' . $numVersao . '"></script>
...
<script type="text/javascript" charset="iso-8859-1" src="' . $strDiretorioJavascriptGlobal . '/calendario/InfraCalendario.js?' . $numVersao . '"></script>
...
';
  ...
  public function abrirBody($strAtributos = '') {
    ...
    $strStyle = '';
    if ($this->getTipoPagina()!=self::$TIPO_PAGINA_COMPLETA || $this->getStrMenuSistema()==null || $this->getStrCookieMenuMostrar()=='N') {
  ...
  protected function montarBarraAcesso() {
    if ($this->isBolMontarBarraAcesso() && $this->getObjInfraSessao()!=null && $this->getObjInfraSessao()->getDthUltimoAcesso()!=null && $this->recuperarSessao('infra_global', self::$POS_SES_ULTIMO_ACESSO)==null) {

      $strTexto = InfraData::formatarExtenso4($this->getObjInfraSessao()->getDthUltimoAcesso());
      $strTexto = 'Último acesso '.(strpos($strTexto, 'feira') != null ? 'na' : 'no').' '.InfraString::transformarCaixaBaixa($strTexto);

      if ($this->getObjInfraSessao()->verificarPermissao('infra_acesso_usuario_listar')) {
        $strTexto = '<a title="Ver últimos acessos" href="'.$this->getObjInfraSessao()->assinarLink('controlador.php?acao=infra_acesso_usuario_listar').'">'.$strTexto.'</a>';
      }

      echo '<div id="divInfraBarraAcesso" class="infraBarraAcesso"><span>'.$strTexto.'</span></div>'."\n";

      $this->adicionarSessao('infra_global', self::$POS_SES_ULTIMO_ACESSO, true);
    }
  }

  private function montarBarraTribunal() {
    if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA || $this->getTipoPagina()==self::$TIPO_PAGINA_SEM_MENU) {
      $linkTRF = '<a href="http://www.trf4.gov.br" target="_blank" title="Site do Tribunal Regional Federal da 4ª Região" tabindex="' . $this->getProxTabBarraTribunal() . '">Tribunal Regional Federal da 4ª Região</a>' . "\n";
      $linkPR = '<a href="http://www.jfpr.gov.br" target="_blank" title="Site da Justiça Federal do Paraná" tabindex="' . $this->getProxTabBarraTribunal() . '">PR</a>' . "\n";
      $linkRS = '<a href="http://www.jfrs.gov.br" target="_blank" title="Site da Justiça Federal do Rio Grande do Sul" tabindex="' . $this->getProxTabBarraTribunal() . '">RS</a>' . "\n";
      $linkSC = '<a href="http://www.jfsc.gov.br" target="_blank" title="Site da Justiça Federal de Santa Catarina" tabindex="' . $this->getProxTabBarraTribunal() . '">SC</a>' . "\n";
  ...
  public function abrirAreaTabela($strSelecao = 'Infra') {
    echo '<div id="div' . $strSelecao . 'AreaTabela' . $this->getProxNumAreaTabela($strSelecao) . '" class="infraAreaTabela">' . "\n";
  }
  ...
  public function montarAreaTabela($strResultado, $numRegistros, $bolExecutou = false, $strAtributos = '', $arrSelecoes = null, $strSelecaoPrincipal = 'Infra', $bolForcarMontagemConteudo = false) {
    ...
    if ($this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PREPARAR] && !$this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PROCESSAR]) {
      $this->adicionarMensagem('Falta processar paginação do conjunto ' . $strSelecaoPrincipal . '.', self::$TIPO_MSG_ERRO);
    } else if (!$this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PREPARAR] && $this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PROCESSAR]) {
      $this->adicionarMensagem('Falta preparar paginação do conjunto ' . $strSelecaoPrincipal . '.', self::$TIPO_MSG_ERRO);
    }

    if ($this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PREPARAR] && $this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PROCESSAR]) {
      $this->montarAreaPaginacao('Superior', $strSelecaoPrincipal, $this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_TAB_INDEX]);
    }

    echo '<div id="div' . $strSelecaoPrincipal . 'AreaTabela' . $this->getProxNumAreaTabela($strSelecaoPrincipal) . '" class="infraAreaTabela" ' . $strAtributos . '>' . "\n";
    ...
    if (!$this->bolMontouTabela) {
      ...
      foreach ($arrSel as $strNomeSelecao) {
        ...
        if ($this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_SESSAO]) {
          ...
          if ($this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_ITENS_SELECIONADOS]!='') {
            ...
            if ($this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_ITENS]=='') {
              ...
              $arrSelecionados = explode(',', $this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_ITENS_SELECIONADOS]);
              $strTemp = '';
        ...
        echo "\n" . '<input type="hidden" id="hdn' . $strNomeSelecao . 'NroItens" name="hdn' . $strNomeSelecao . 'NroItens" value="' . $this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_NRO_ITENS] . '" />' . "\n" .
            ...
            '<input type="hidden" id="hdn' . $strNomeSelecao . 'ItensHash" name="hdn' . $strNomeSelecao . 'ItensHash" value="' . $this->gerarHashConteudo($this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_ITENS]) . '" />' . "\n" .
      ...
      echo "\n" . '<input type="hidden" id="hdnInfraSelecoes" name="hdnInfraSelecoes" value="' . implode(',', array_keys($this->arrSelecoes)) . '" />' . "\n";
    }
    ...
    if (isset($this->arrSelecoes[$strSelecaoPrincipal])) {
      ...
      if ($this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PREPARAR] && $this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PROCESSAR]) {
        $this->montarAreaPaginacao('Inferior', $strSelecaoPrincipal, $this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_TAB_INDEX]);
  ...
  private function montarAreaPaginacao($strTipo, $strSelecao, $bolMontarTabIndexPaginacao = true) {

    echo '<div id="div' . $strSelecao . 'AreaPaginacao' . $strTipo . '" class="infraAreaPaginacao">' . "\n";

    //Se tem mais de uma página
    if ($this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_TOTAL_REGISTROS]>$this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_REGISTROS_PAGINA_ATUAL]) {

      $numPaginas = ceil($this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_TOTAL_REGISTROS] / $this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_REGISTROS_POR_PAGINA]);

      //Se não esta na primeira página
      if ($this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_PAGINA_ATUAL]>0) {
        if ($numPaginas>2) {
          echo '<a id="lnk' . $strSelecao . 'PrimeiraPagina' . $strTipo . '" href="javascript:void(0);" onclick="infraAcaoPaginar(\'=\',0,\'' . $strSelecao . '\');" title="Primeira Página" ' . ($bolMontarTabIndexPaginacao ? 'tabindex="' . $this->getProxTabBarraSistema() . '"' : '') . '><img src="' . $this->getDiretorioImagensGlobal() . '/primeira_pagina.gif" title="Primeira Página" alt="Primeira Página" class="infraImg" /></a>&nbsp;&nbsp;' . "\n";
        } else {
          echo str_repeat('&nbsp;', 7);
        }
        echo '<a id="lnk' . $strSelecao . 'PaginaAnterior' . $strTipo . '" href="javascript:void(0);" onclick="infraAcaoPaginar(\'-\',0,\'' . $strSelecao . '\');" title="Página Anterior" ' . ($bolMontarTabIndexPaginacao ? 'tabindex="' . $this->getProxTabBarraSistema() . '"' : '') . '><img src="' . $this->getDiretorioImagensGlobal() . '/pagina_anterior.gif" title="Página Anterior" alt="Página Anterior" class="infraImg" /></a>&nbsp;&nbsp;' . "\n";
      } else {
        echo str_repeat('&nbsp;', 14);
      }

      if ($numPaginas>2) {
        echo '<select id="sel' . $strSelecao . 'Paginacao' . $strTipo . '" name="sel' . $strSelecao . 'Paginacao' . $strTipo . '" onchange="infraAcaoPaginar(\'=\',this.value,\'' . $strSelecao . '\');" class="infraSelect" ' . ($bolMontarTabIndexPaginacao ? 'tabindex="' . $this->getProxTabBarraSistema() . '"' : '') . ' style="display:inline;">' . "\n";
        for ($i = 0; $i<$numPaginas; $i ++) {
          echo '<option value="' . $i . '"';
          if ($i==$this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_PAGINA_ATUAL]) {
      ...
      if ($this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_PAGINA_ATUAL]<($numPaginas - 1)) {
        echo '<a id="lnk' . $strSelecao . 'ProximaPagina' . $strTipo . '" href="javascript:void(0);" onclick="infraAcaoPaginar(\'+\',0,\'' . $strSelecao . '\');" title="Próxima Página" ' . ($bolMontarTabIndexPaginacao ? 'tabindex="' . $this->getProxTabBarraSistema() . '"' : '') . '><img src="' . $this->getDiretorioImagensGlobal() . '/proxima_pagina.gif" title="Próxima Página" alt="Próxima Página" class="infraImg" /></a>&nbsp;&nbsp;' . "\n" . '&nbsp;';
        if ($numPaginas>2) {
          echo '<a id="lnk' . $strSelecao . 'UltimaPagina' . $strTipo . '" href="javascript:void(0);" onclick="infraAcaoPaginar(\'=\',' . ($numPaginas - 1) . ',\'' . $strSelecao . '\');" title="Última Página" ' . ($bolMontarTabIndexPaginacao ? 'tabindex="' . $this->getProxTabBarraSistema() . '"' : '') . '><img src="' . $this->getDiretorioImagensGlobal() . '/ultima_pagina.gif" title="Última Página" alt="Última Página" class="infraImg" /></a>' . "\n";
        }
      }
    }
    echo '</div>' . "\n";
  ...
  public function montarMensagens(){
    ...
    if ($this->getBolExibirMensagens()) {
      ...
      if (isset($_GET['msg'])) {
        if ($_GET['msg']!='') {
          echo '<textarea id="txaInfraMsg" name="txaInfraMsg" style="display:none">' . str_replace('\n', "\n", self::tratarHTML($_GET['msg'])) . '</textarea>' . "\n";
          $strAlert .= 'self.setTimeout(\'alert(document.getElementById(\\\'txaInfraMsg\\\').value)\',100);';
        }
      }

      $strMensagens = $this->getStrMensagens();
      if ($strMensagens!='') {
        echo '<textarea id="txaInfraValidacao" name="txaInfraValidacao" style="display:none">' . str_replace('\n', "\n", self::tratarHTML($strMensagens)) . '</textarea>' . "\n";
        $strAlert .= 'self.setTimeout(\'alert(document.getElementById(\\\'txaInfraValidacao\\\').value)\',100);';
      }
  ...
  protected function complementarAtributo($strAtributos, $strTagAtributo, $strComplementacao) {
    ...
    if ($strAtributos!='') {
      ...
      if ($posTagAtributo!==false) {
        ...
        $strAtributos = substr($strAtributos, 0, $posTagAtributo + strlen($strTagAtributo . '="')) . $strComplementacao . substr($strAtributos, $posTagAtributo + strlen($strTagAtributo . '="'));
      } else {
  ...
  public function getThCheck($strRotulo = '', $strNomeSelecao = 'Infra', $strAtributos = '', $bolGerarTabIndex = true) {
    ...
    $this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_TAB_INDEX] = $bolGerarTabIndex;
    ...
    if (isset($_GET['acao_origem']) && $_GET['acao_origem']!='') {
      ...
      $this->adicionarSessao($_GET['acao'], self::$POS_SES_GRUPOS_SELECOES, null);
    }
    ...
    //$strImagem = $this->getDiretorioImagens().'/check.gif';
    $strImagem = $this->getDiretorioImagensGlobal() . '/check.gif';

    $strAtributos = $this->complementarAtributo($strAtributos, 'onclick', 'infraSelecaoMultipla(\'' . $strNomeSelecao . '\');');

    $strAnchor = '';
    $strAnchor .= '<label id="lblInfraCheck" for="lnkInfraCheck" accesskey=";"></label>' . "\n";
    $strAnchor .= '<a href="javascript:void(0);" id="lnkInfraCheck" ' . $strAtributos . ' ' . ($bolGerarTabIndex ? 'tabindex="' . $this->getProxTabTabela() . '"' : '') . '><img src="' . $strImagem . '" id="img' . $strNomeSelecao . 'Check" title="Selecionar Tudo" alt="Selecionar Tudo" class="infraImg" /></a>';
  ...
  public function getTrCheck($numItem, $strId, $strTitulo, $strValor = 'N', $strNomeSelecao = 'Infra', $strAtributos = '') {
    ...
    if ($this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_SESSAO]) {
      ...
      if (InfraUtil::inArray($strId, explode(',', $this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_ITENS_SELECIONADOS]))) {
    ...
    if ($this->getTipoSelecao()==self::$TIPO_SELECAO_SIMPLES) {
      $strType = 'radio';
      $strTagId = 'chk' . $strNomeSelecao . 'Item' . $numItem;
      $strTagName = 'chk' . $strNomeSelecao . 'Item';
    } else {
      $strType = 'checkbox';
      $strTagId = 'chk' . $strNomeSelecao . 'Item' . $numItem;
      $strTagName = $strTagId;
    ...
    $strAtributos = $this->complementarAtributo($strAtributos, 'onclick', 'infraSelecionarItens(this,\'' . $strNomeSelecao . '\');');

    $ret = '';

    $ret .= '<a id="lnk' . $strNomeSelecao . 'ID-' . $strId . '" name="ID-' . $strId . '"></a>';

    $ret .= '<input ' . $strCheck . ' type="' . $strType . '" id="' . $strTagId . '"  name="' . $strTagName . '" ' . $strAtributos . ' class="infraCheckbox" value="' . $strId . '" title="' . self::tratarHTML($strTitulo) . '" ' . ($this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_TAB_INDEX] ? 'tabindex="' . $this->getProxTabTabela() . '"' : '') . ' />';

    return $ret;
  }

  public function getAcaoTransportarItem($numItem, $strId, $strNomeSelecao = 'Infra', $strAtributos = '', $strTitulo = 'Transportar este item e Fechar', $strIcone = null) {
    if ($this->isBolPaginaSelecao()) {
      $strAtributos = $this->complementarAtributo($strAtributos, 'onclick', 'infraTransportarItem(' . $numItem . ',\'' . $strNomeSelecao . '\');');

      if ($strIcone==null) {
        $strIcone = $this->getDiretorioImagensGlobal() . '/transportar.gif';
      }

      return '<a id="lnk' . $strNomeSelecao . 'T-' . $strId . '" href="#" ' . $strAtributos . ' tabindex="' . $this->getProxTabTabela() . '"><img src="' . $strIcone . '" title="' . $strTitulo . '" alt="' . $strTitulo . '" class="infraImg" /></a>&nbsp;';
    }
  }
  ...
  public function getArrStrItensSelecionados($strNomeSelecao = 'Infra', $bolManterOrdemMarcacao = true) {
    ...
    } else if (isset($_POST['hdn' . $strNomeSelecao . 'ItensSelecionados']) && $_POST['hdn' . $strNomeSelecao . 'ItensSelecionados']!='') {
    ...
    if ($this->validarHashTabelas() && InfraArray::contar($ret)) {
      ...
      if ($_POST['hdn' . $strNomeSelecao . 'ItensHash']!=$this->gerarHashConteudo($_POST['hdn' . $strNomeSelecao . 'Itens'])) {
  ...
  public function getArrItensTabelaDinamica($strDados) {
    $ret = array();
    if ($strDados!='') {
  ...
  public function gerarItensTabelaDinamica($arr) {
    $ret = '';
    if (is_array($arr)) {
      $numLinhas = count($arr);
  ...
  public function processarExcecao($e, $bolLimparParametrosLog = false) {
    ...
    if ($e===null) {
      return;
    } else if ($e instanceof InfraException) {
        ...
        if (isset($_GET['acao_origem']) && $_GET['acao_origem']!='') {
          ...
          if ($this->getObjInfraSessao()!==null) {
            ...
            if (isset($_POST['hdnInfraSelecoes']) && $_POST['hdnInfraSelecoes']!='') {
              ...
              $this->adicionarSessao($_GET['acao_origem'], self::$POS_SES_GRUPOS_SELECOES, $arrGruposSelecoes);
            }
          }
        }
  ...
  public function prepararBarraProgresso($strTituloJanela, $strTituloPagina = null, $bolExibirCancelar = false) {
    ...
    if ($this instanceof InfraPaginaEsquema || $this instanceof InfraPaginaEsquema2) {
      echo '<link href="' . $this->getDiretorioCssGlobal() . '/infra-global-esquema.css?' . $this->getNumVersao() . '" rel="stylesheet" type="text/css" media="all" />' . "\n";
    } else {
      echo '<link href="' . $this->getDiretorioCssGlobal() . '/infra-global.css?' . $this->getNumVersao() . '" rel="stylesheet" type="text/css" media="all" />' . "\n";
    }

    echo '<link href="' . $this->getDiretorioCssGlobal() . '/infra-barra-progresso.css?' . $this->getNumVersao() . '" rel="stylesheet" type="text/css" media="all" />' . "\n";
  ...
  public function prepararBarraProgresso2($strTituloJanela, $strTituloPagina = null, $bolExibirCancelar = false, $delay = 1000) {
    ...
    echo '<link href="' . $this->getDiretorioCssGlobal() . '/infra-barra-progresso.css?' . $this->getNumVersao() . '" rel="stylesheet" type="text/css" media="all" />' . "\n";
    echo '<link href="' . $this->getDiretorioJavaScriptGlobal() . '/jquery/jquery-ui-1.11.1/jquery-ui.min.css?' . $this->getNumVersao() . '" rel="stylesheet" type="text/css" media="all" />' . "\n";
    $this->abrirStyle();
    ?>
    .ui-progressbar { position: relative; }

    .progress-label { position: absolute; text-align:center; width:100%; top: 4px; font-weight: bold; color:black; text-shadow: 1px 1px 0 white;}
              ...
              if (progressArray = transport.responseJSON) { // set, not compare
                ...
                for (var name in progressArray) {
                  if (progressArray[name].excecao!='') {
                    ...
                    if (progressArray[name].validacao!='') {
                      ...
                      if (progressArray[name].redirect) {
                        this.stop();
//                      alert('redirect');
                        window.opener.location = progressArray[name].redirect;
                      }
...
//                      alert('close');
  ...
  public function finalizarBarraProgresso($strDestinoJanelaPai = null, $bolFecharAutomaticamente = true) {
    ...
    if ($this->getStrMensagens()=='' && $_GET['msg']=='') {
      ...
      if ($strDestinoJanelaPai!=null) {
        echo '  window.opener.location=\'' . $strDestinoJanelaPai . '\';' . "\n";
      }
  ...
  public function montarPaginaErro($strErro, $strDetalhes, $strTrace) {
    ...
    if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA || $this->getTipoPagina()==self::$TIPO_PAGINA_SEM_MENU) {
      $arrComandos[] = '<input type="button" id="btnInfraVoltarExcecao" name="btnInfraVoltarExcecao" value="Voltar" onclick="javascript:history.go(-1);" class="infraButton" />';
    } else {
      $arrComandos[] = '<input type="button" id="btnInfraFecharExcecao" name="btnInfraFecharExcecao" value="Fechar" onclick="window.close();" class="infraButton" />';
    }
    $this->montarBarraComandosSuperior($arrComandos);
  ...
  public function montarErroBarraProgresso($strErro, $strDetalhes, $strTrace) {
    ...
    $strComandos .= $this->montarBotaoFecharExcecao();
    $strComandos .= '\';' . "\n";
    ...
    if (!$this->isBolProducao()) {
      ...
      if ($strDetalhes!='') {
        $strConteudoDetalhes .= '<br /><br /><b>Detalhes:</b><br />' . nl2br(self::tratarHTML(str_replace(',', ', ', $strDetalhes)));
      }

      if ($strTrace!='') {
        $strTrace = str_replace("\\n", '', $strTrace);
        $strConteudoDetalhes .= '<br /><br /><b>Trilha de Processamento:</b><br />' . nl2br(self::tratarHTML(InfraString::limparParametrosPhp($strTrace)));
      }
    ...
    $this->abrirJavaScript();
    echo $strComandos;
    echo 'document.getElementById(\'spnDetalhesExcecao\').innerHTML = (new infraBase64()).decodificar(\'' . base64_encode($strConteudoDetalhes) . '\');';
    $this->fecharJavaScript();
  ...
  public function gerarCaptionTabela($strEntidade, $numRegistros = null, $strPrefixo = 'Lista de ', $strSelecao = 'Infra') {
   ...
   */
  public function prepararOrdenacao($objInfraDTO, $strCampoOrdDefault, $strTipoOrdDefault, $bolIgnorarSessao = false, $strSelecao = 'Infra') {
    ...
    if (isset($_POST['hdn' . $strSelecao . 'CampoOrd'], $_POST['hdn' . $strSelecao . 'TipoOrd']) &&
      ...
      $this->adicionarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_ORDENACAO_CAMPO, $this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_CAMPO]);
      $this->adicionarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_ORDENACAO_TIPO, $this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_TIPO]);
    ...
    } else if (!$bolIgnorarSessao && ($this->recuperarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_ORDENACAO_TIPO)!='' &&
            $this->recuperarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_ORDENACAO_CAMPO)!='')) {
      $this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_CAMPO] = $this->recuperarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_ORDENACAO_CAMPO);
      $this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_TIPO] = $this->recuperarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_ORDENACAO_TIPO);
      $objInfraDTO->setOrd($this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_CAMPO], $this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_TIPO]);
  ...
  public function prepararPaginacao($objInfraDTO, $numRegistrosPorPagina = 50, $bolIgnorarSessao = false, $numPaginaAtual = null, $strSelecao = 'Infra') {
    ...
    if ($this->getObjInfraSessao()==null) {
      $strHashAtual = md5($objInfraDTO->__toString());
    } else {
      $strHashAtual = md5($objInfraDTO->__toString() . $this->getObjInfraSessao()->getNumIdUnidadeAtual());
    }
    ...
    if (isset($_POST['hdn' . $strSelecao . 'PaginaAtual']) && $_POST['hdn' . $strSelecao . 'PaginaAtual']!='') {
      ...
      //salva na sessao
      $this->adicionarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_PAGINA_ATUAL, $this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_PAGINA_ATUAL]);
      $this->adicionarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_HASH_CRITERIOS, $this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_HASH_CRITERIOS]);

      //2 - Chegou pelo menu ou por outra pagina - verifica na sessão
    } else if (!$bolIgnorarSessao && $this->recuperarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_PAGINA_ATUAL)!='') {

      $this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_PAGINA_ATUAL] = $this->recuperarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_PAGINA_ATUAL);
      $this->arrSelecoes[$strSelecao][self::$POS_SEL_PAG_HASH_CRITERIOS] = $this->recuperarSessao($_GET['acao'] . '-' . $strSelecao, self::$POS_SES_HASH_CRITERIOS);
   ...
   * @throws InfraException
   */
  public function getThOrdenacao($objInfraDTO, $strRotulo, $strCampo, &$arrObjInfraDTO, $bolForcarOrdenacaoMemoria = false, $strSelecao = 'Infra') {

    $strTh = '';
    $strTh .= '<table class="infraTableOrdenacao">';
    $strTh .= '<tr class="infraTrOrdenacao">';
    $strTh .= '<td width="1%" class="infraTdSetaOrdenacao">';

    $strRotuloBak = $strRotulo;
    $strRotulo = str_replace('<br />', '', $strRotulo);

    if ($this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_CAMPO]!=$strCampo || $this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_TIPO]!=InfraDTO::$TIPO_ORDENACAO_ASC) {
      $strImagem = $this->getDiretorioImagensGlobal() . '/seta_acima.gif';
    } else {
      $strImagem = $this->getDiretorioImagensGlobal() . '/seta_acima_selecionada.gif';
    ...
    $strTh .= '<a href="javascript:void(0);" onclick="infraAcaoOrdenar(\'' . $strCampo . '\',\'' . InfraDTO::$TIPO_ORDENACAO_ASC . '\',\'' . $strSelecao . '\');" ' . ($this->arrSelecoes[$strSelecao][self::$POS_SEL_TAB_INDEX] ? 'tabindex="' . $this->getProxTabTabela() . '"' : '') . '><img src="' . $strImagem . '" title="Ordenar ' . $strRotulo . ' Ascendente" alt="Ordenar ' . $strRotulo . ' Ascendente" class="infraImgOrdenacao" /></a>';
    if ($this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_CAMPO]!=$strCampo || $this->arrSelecoes[$strSelecao][self::$POS_SEL_ORD_TIPO]!=InfraDTO::$TIPO_ORDENACAO_DESC) {
      $strImagem = $this->getDiretorioImagensGlobal() . '/seta_abaixo.gif';
    } else {
      $strImagem = $this->getDiretorioImagensGlobal() . '/seta_abaixo_selecionada.gif';
      //Verifica se o campo foi ordenado pelo banco (se era atributo de banco, o banco ordena)
      $arrAtributos = $objInfraDTO->getArrAtributos();
      if ($arrAtributos[$strCampo][InfraDTO::$POS_ATRIBUTO_CAMPO_SQL]==null || $bolForcarOrdenacaoMemoria) {
        //ordenar (para casos onde o atributo não é de banco)
    ...
    $strTh .= '</td>';
    $strTh .= '<td rowspan="2" valign="center" class="infraTdRotuloOrdenacao">' . $strRotuloBak . '</td>';
    $strTh .= '</tr>';
    $strTh .= '<tr class="infraTrOrdenacao"><td class="infraTdSetaOrdenacao">';
    $strTh .= '<a href="javascript:void(0);" onclick="infraAcaoOrdenar(\'' . $strCampo . '\',\'' . InfraDTO::$TIPO_ORDENACAO_DESC . '\',\'' . $strSelecao . '\');" ' . ($this->arrSelecoes[$strSelecao][self::$POS_SEL_TAB_INDEX] ? 'tabindex="' . $this->getProxTabTabela() . '"' : '') . '><img src="' . $strImagem . '" title="Ordenar ' . $strRotulo . ' Descendente" alt="Ordenar ' . $strRotulo . ' Descendente" class="infraImgOrdenacao" /></a>';
    $strTh .= '</td>';
    $strTh .= '</tr>';
    $strTh .= '</table>';
  ...
  public function montarMenuSessao($strNomeMenu) {
    if ($this->getObjInfraSessao()!==null) {
      ...
      return ($this->tipoMenu==self::$MENU_NORMAL ? $this->montarMenuArray($arrMenu) : $this->montarSmartMenuArray($arrMenu));
    }
    return '';
  }
  ...
  public function montarSmartMenuArray($arrMenu, $n = '') {
    ...
    if ($numLimite>0) {
      $strMenu .= '<ul id="main-menu" class="sm sm-vertical sm-' . $this->obterSmartMenuClass() . ' sm-' . $this->obterSmartMenuClass() . '-vertical">' . "\n";
    }
  ...
  public function montarLinkSair($strLink = null, $strIcone = null) {
    ...
    $str .= '" title="Sair do Sistema"  tabindex="' . $this->getProxTabBarraSistema() . '"><img src="' . $strIcone . '" title="Sair do Sistema" alt="Sair do Sistema" class="infraImg" /></a>';
    return $str;
  }
  ...
  public function montarLinkAjuda($strLink, $strIcone = null) {
    ...
    if ($strLink!=null) {
      ...
      if ($strIcone==null) {
        $strIcone = $this->getDiretorioImagensGlobal() . '/ajuda.gif';
      }


      $str = '<a id="lnkAjudaSistema" href="' . $strLink . '" target="_blank" title="Ajuda"  tabindex="' . $this->getProxTabBarraSistema() . '"><img src="' . $strIcone . '" title="Ajuda" alt="Ajuda" class="infraImg" /></a>';
    }
    return $str;
  }
  ...
  public function verificarSelecao($strAcao) {
    if (isset($_GET['acao_origem'])) {
      ...
      } else if ($_GET['acao_origem']==$_GET['acao'] && isset($_POST['hdnInfraTipoPagina'])) {
  ...
  public function montarLinkMenu() {
    if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA) {
      return '<a id="lnkMenuSistema" onclick="infraMenuSistema(false);" title="Exibir/Ocultar Menu do Sistema" tabindex="' . $this->getProxTabBarraSistema() . '"><img src="' . $this->getDiretorioImagensGlobal() . '/menu.gif" title="Exibir/Ocultar Menu do Sistema" alt="Exibir/Ocultar Menu do Sistema" class="infraImg" /></a>';
    }
  }
  ...
  public function montarLinkUsuario($strSigla = null, $strOrgao = null, $strNome = null, $strIcone = null) {
    ...
    return '<a id="lnkUsuarioSistema" '.$strLinkAcessos.' title="' . $strDados . '" tabindex="' . $this->getProxTabBarraSistema() . '"><img src="' . $strIcone . '" title="' . $strDados . '" alt="' . $strDados . '" class="infraImg" /></a>';
  }
  ...
  public function montarSelectUnidades() {
    ...
    if ($objInfraSessao!==null) {
      ...
      $objInfraSessao->setStrUltimaPagina(str_replace('infra_sip=true', 'infra_sip=false', $_SERVER['REQUEST_URI']));

      //$str .= '<!-- '.$objInfraSessao->getStrUltimaPagina().' -->'."\n";
      $str .= "\n";
      $str .= '<form id="frmInfraSelecionarUnidade" method="post" action="' . $objInfraSessao->assinarLink($objInfraSessao->getStrPaginaInicial()) . '" style="display:inline;">' . "\n";
      ...
      if ($objInfraSessao->getObjInfraSessaoDTO()!==null) {
        ...
        foreach ($arrIdUnidades as $numIdUnidade) {
          ...
          $str .= '>' . $objInfraSessao->getStrSiglaUnidade($numIdUnidade) . '</option>' . "\n";
        }
      }
      $str .= '</select>' . "\n";
      $str .= '</form>' . "\n";
          ...
          if (!isset($_FILES[$strCampoArquivo])) {
            ...
            if ($_FILES[$strCampoArquivo]['error']!=UPLOAD_ERR_OK) {
              ...
              if (function_exists('finfo_open')) {
                $bolConteudoPermitido = InfraUtil::verificarConteudoPermitidoArquivo($_FILES[$strCampoArquivo]['tmp_name']);
              }
  ...
  public static function montarAncora($var) {
    ...
    } else if (!InfraString::isBolVazia($var)) {
      $strAncora = '#ID-' . $var;
    }
  ...
  public static function montarHeaderDownload($strNomeArquivo = null, $strContentDisposition = 'inline', $strContentType = null, $bolXSS = false) {
    ...
    if ($strNomeArquivo!=null) {
      ...
      header('Content-Disposition: ' . $strContentDisposition . '; filename="' . InfraUtil::formatarNomeArquivo($strNomeArquivo) . '"');
    }
  }
