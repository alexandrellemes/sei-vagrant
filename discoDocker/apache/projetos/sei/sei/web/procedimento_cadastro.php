...
try {
  ...
  switch($_GET['acao']){
    ...
    case 'procedimento_gerar_relacionado':
      ...
      if ($_POST['hdnFlagProcedimentoCadastro']=='2'){
        try{
          ...
          header('Location: '.SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$strAcao.'&acao_origem='.$_GET['acao'].'&acao_retorno='.PaginaSEI::getInstance()->getAcaoRetorno().'&id_procedimento='.$objProcedimentoDTO->getDblIdProcedimento().$strParametros.'&atualizar_arvore=1'.PaginaSEI::getInstance()->montarAncora($objProcedimentoDTO->getDblIdProcedimento())));
          die;
        }catch(Exception $e){
    ...
    case 'procedimento_alterar':    	
      ...
      if ($_POST['hdnFlagProcedimentoCadastro']=='2'){
        try{          
          ...
          if (isset($_GET['id_procedimento_retorno'])){
            $strParametros = str_replace('&id_procedimento='.$objProcedimentoDTO->getDblIdProcedimento(),'&id_procedimento='.$_GET['id_procedimento_retorno'].'&id_procedimento_anexado='.$objProcedimentoDTO->getDblIdProcedimento(),$strParametros);
          }

          header('Location: '.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=arvore_visualizar&acao_origem='.$_GET['acao'].$strParametros.'&atualizar_arvore=1'.PaginaSEI::getInstance()->montarAncora($objProcedimentoDTO->getDblIdProcedimento())));
          die;
    ...
    case 'procedimento_consultar':
      ...
      $arrComandos[] = '<button type="button" accesskey="F" name="btnFechar" value="Fechar" onclick="location.href=\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.PaginaSEI::getInstance()->getAcaoRetorno().'&acao_origem='.$_GET['acao'].$strParametros.PaginaSEI::getInstance()->montarAncora($strAncora)).'\';" class="infraButton"><span class="infraTeclaAtalho">F</span>echar</button>';
  ...
  if (!isset($_POST['hdnFlagProcedimentoCadastro'])){
    if ($_GET['acao']=='procedimento_gerar' || $_GET['acao']=='procedimento_gerar_relacionado'){
      $strItensSelRelProtocoloAssunto = TipoProcedimentoINT::montarSelectSugestaoAssuntosRI0567($objProcedimentoDTO->getNumIdTipoProcedimento());
    }else{
      $strItensSelRelProtocoloAssunto = RelProtocoloAssuntoINT::conjuntoPorCodigoDescricaoRI0510($objProcedimentoDTO->getDblIdProcedimento());
  ...
  }else if ($_POST['hdnFlagProcedimentoCadastro']=='1'){
    $_POST['hdnAssuntos'] = '';
    $strItensSelRelProtocoloAssunto = AssuntoINT::montarSelectTrocaTipoProcedimento($objProcedimentoDTO->getNumIdTipoProcedimento(), $arrAssuntos);
  }
  
  
  //INTERESSADOS
  $strItensSelParticipante = ParticipanteINT::conjuntoPorParticipacaoRI0513($objProcedimentoDTO->getDblIdProcedimento(),array(ParticipanteRN::$TP_INTERESSADO));
  ...
  if ($_GET['acao']=='procedimento_gerar' || $_GET['acao']=='procedimento_gerar_relacionado'){
    if (($bolNumeroProcessoInformar = $objProcedimentoRN->verificarLiberacaoNumeroProcesso())){
    ...
    $bolNumeroProcessoAlterar = $objProcedimentoRN->verificarLiberacaoNumeroProcesso();
  }else{
  ...
  if ($strSinIndividual == 'S'){
    if (!isset($_POST['hdnIdProcedimento'])){  
      ...
      }else if (count($arrObjParticipanteDTO) == 1){
  ...
  $strLinkAjaxInteressadoUsuario = SessaoSEI::getInstance()->assinarLink('controlador_ajax.php?acao_ajax=usuario_auto_completar_contato');
  ...
  $strItensSelTipoProcedimento = TipoProcedimentoINT::montarSelectNomeLiberados('null','&nbsp;',$objProcedimentoDTO->getNumIdTipoProcedimento(), true);
...
#selTipoProcedimento {position:absolute;left:0%;top:40%;width:90%;}

<? if ($bolNumeroProcessoInformar){ ?>
#divProtocoloInformar {}
#fldProtocoloInformar {position:absolute;left:0%;top:0%;height:80%;width:88%;}
...
#lblProtocoloInformar {position:absolute;left:21%;top:38%;width:34%;<?=$strExibirProtocoloParaDigitacao?>}
#txtProtocoloInformar {position:absolute;left:21%;top:60%;width:34%;<?=$strExibirProtocoloParaDigitacao?>}

#lblDtaGeracaoInformar {position:absolute;left:60%;top:38%;width:25%;<?=$strExibirProtocoloParaDigitacao?>}
#txtDtaGeracaoInformar {position:absolute;left:60%;top:60%;width:12%;<?=$strExibirProtocoloParaDigitacao?>}
#imgCalDtaGeracaoInformar {position:absolute;left:73%;top:60%;<?=$strExibirProtocoloParaDigitacao?>}
<?}?>
...
#txtDtaGeracaoAlterar {position:absolute;left:33%;top:40%;width:16%;}
<?}?>
...
#txtDescricao {position:absolute;left:0%;top:40%;width:90%;}
...
#selInteressadosProcedimento {position:absolute;left:0%;top:38%;width:90%;height:56%;}
...
#txtInteressadoUsuario {position:absolute;left:0%;top:40%;width:89.5%;}
...
?>
  ...
  #lblProtocoloInformar {top:25%;}
  #txtProtocoloInformar {top:50%;}

  #lblDtaGeracaoInformar {top:25%;}
...
function inicializar(){
  ...
  objAutoCompletarInteressadoUsuario.selecionar('<?=$strIdInteressadoUsuario?>','<?=PaginaSEI::getInstance()->formatarParametrosJavaScript($strNomeInteressadoUsuario);?>');
  ...
  objLupaAssuntos.processarAlteracao = function (pos, texto, valor){
    seiConsultarAssunto(valor, 'selAssuntos','frmProcedimentoCadastro','<?=$strLinkConsultarAssunto?>');
  }
  ...
  objLupaInteressados.processarAlteracao = function (pos, texto, valor){
    seiAlterarContato(valor, 'selInteressadosProcedimento', 'frmProcedimentoCadastro','<?=$strLinkAlterarContato?>');
  }
...
?>
<form id="frmProcedimentoCadastro" method="post" onsubmit="return OnSubmitForm();" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'].$strParametros)?>" style="display:inline;">
<?
...
?>
  <? if ($bolNumeroProcessoInformar){ ?>
    <div id="divProtocoloInformar" class="infraAreaDados" style="height:9em;">
      ...
      <fieldset id="fldProtocoloInformar" class="infraFieldset">
        <legend class="infraLegend">&nbsp;Protocolo&nbsp;</legend>
          ...
          <div id="divOptProtocoloAutomatico" class="infraDivRadio">
            <input type="radio" name="rdoProtocolo" id="optProtocoloAutomatico" onclick="alterarProtocolo()" value="A" <?=($bolMarcarProtocoloAutomatico?'checked="checked"':'')?> class="infraRadio"/>
            <span id="spnProtocoloAutomatico"><label id="lblProtocoloAutomatico" for="optProtocoloAutomatico" class="infraLabelRadio" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">Automático</label><label>&nbsp;</label></span>
          </div>

          <div id="divOptProtocoloManual" class="infraDivRadio">
            <input type="radio" name="rdoProtocolo" id="optProtocoloManual" onclick="alterarProtocolo()" value="M" <?=($bolMarcarProtocoloManual?'checked="checked"':'')?> class="infraRadio"/>
            <span id="spnProtocoloManual"><label id="lblProtocoloManual" for="optProtocoloManual" class="infraLabelRadio" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">Informado</label></span>
          </div>

          <label id="lblProtocoloInformar" for="txtProtocoloInformar" accesskey="" class="infraLabelObrigatorio">Número:</label>
          <input type="text" id="txtProtocoloInformar" name="txtProtocoloInformar"  <?=(InfraString::isBolVazia($strMascaraProtocolo)?'':'onkeypress="return infraMascara(this,event,\''.$strMascaraProtocolo.'\');"')?> class="infraText" value="<?=PaginaSEI::tratarHTML($objProtocoloDTO->getStrProtocoloFormatado());?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" <?=($_POST['rdoProtocolo']=='M'?'style="visibility:visible"':'')?> />

          <label id="lblDtaGeracaoInformar" for="txtDtaGeracaoInformar" class="infraLabelObrigatorio">Data de Autuação:</label>
          <input type="text" id="txtDtaGeracaoInformar" name="txtDtaGeracaoInformar" onkeypress="return infraMascaraData(this, event)" class="infraText" value="<?=PaginaSEI::tratarHTML($_POST['txtDtaGeracaoInformar'])?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
          <img src="<?=PaginaSEI::getInstance()->getDiretorioImagensGlobal()?>/calendario.gif" id="imgCalDtaGeracaoInformar" title="Selecionar Data" alt="Selecionar Data"  class="infraImg" onclick="infraCalendario('txtDtaGeracaoInformar',this);" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
  ...
  <? if ($bolNumeroProcessoAlterar){ ?>
    <div id="divProtocoloAlterar" class="infraAreaDados" style="height:4.5em;">
      <label id="lblProtocoloAlterar" for="txtProtocoloAlterar" accesskey="" class="infraLabelObrigatorio">Protocolo:</label>
      <input type="text" id="txtProtocoloAlterar" name="txtProtocoloAlterar" class="infraText" value="<?=PaginaSEI::tratarHTML($objProtocoloDTO->getStrProtocoloFormatado());?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />

      <label id="lblDtaGeracaoAlterar" for="txtDtaGeracaoAlterar" accesskey="" class="infraLabelObrigatorio">Data de Autuação:</label>
      <input type="text" id="txtDtaGeracaoAlterar" name="txtDtaGeracaoAlterar" onkeypress="return infraMascaraData(this, event)" class="infraText" value="<?=PaginaSEI::tratarHTML($objProtocoloDTO->getDtaGeracao());?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
    </div>
  <? } ?>
  ...
  <? if ($bolNumeroProcessoExibir){ ?>
    <div id="divProtocoloExibir" class="infraAreaDados" style="height:4.5em;">
      ...
      <input type="text" id="txtProtocoloExibir" name="txtProtocoloExibir" class="infraText infraReadOnly" readonly="readonly" value="<?=PaginaSEI::tratarHTML($objProtocoloDTO->getStrProtocoloFormatado());?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />

      <label id="lblDtaGeracaoExibir" for="txtDtaGeracaoExibir" accesskey="" class="infraLabelObrigatorio">Data de Autuação:</label>
      <input type="text" id="txtDtaGeracaoExibir" name="txtDtaGeracaoExibir" class="infraText infraReadOnly" readonly="readonly" value="<?=PaginaSEI::tratarHTML($objProtocoloDTO->getDtaGeracao());?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />

    </div>

    <div id="divTipoProcedimentoTitulo" class="tituloProcessoDocumento" >
      <label id="lblTipoProcedimentoTitulo"><?=PaginaSEI::tratarHTML($objProcedimentoDTO->getStrNomeTipoProcedimento())?></label>
    </div>
  <? } ?>

  <div id="divTipoProcedimento" class="infraAreaDados" style="height:4.5em;">
    <label id="lblTipoProcedimento" for="selTipoProcedimento" accesskey="" class="infraLabelObrigatorio">Tipo do Processo:</label> 
    <select id="selTipoProcedimento" name="selTipoProcedimento" onchange="trocarTipoProcedimento();" class="infraSelect" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" >
    <?=$strItensSelTipoProcedimento?>
    </select>
  </div>

  <div id="divDescricao" class="infraAreaDados" style="height:4.7em;">
    <label id="lblDescricao" for="txtDescricao" accesskey="" class="infraLabelOpcional">Especificação:</label>
    <input type="text" id="txtDescricao" name="txtDescricao" onkeypress="return infraMascaraTexto(this,event,100);" maxlength="100" class="infraText" value="<?=PaginaSEI::tratarHTML($objProtocoloDTO->getStrDescricao())?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
  </div>
  
  <div id="divAssuntos" class="infraAreaDados" style="height:11em;">
    <label id="lblAssuntos" for="txtAssunto" accesskey="u" class="infraLabelObrigatorio">Classificação por Ass<span class="infraTeclaAtalho">u</span>ntos:</label>
  ...
  <div id="divInteressados" class="infraAreaDados" style="height:11em;">
    <label id="lblInteressadosProcedimento" for="txtInteressadoProcedimento" accesskey="I" class="infraLabelOpcional"><span class="infraTeclaAtalho">I</span>nteressados:</label>
    <input type="text" id="txtInteressadoProcedimento" name="txtInteressadoProcedimento" class="infraText" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>"/>
    <input type="hidden" id="hdnIdInteressadoProcedimento" name="hdnIdInteressadoProcedimento" class="infraText" value="" />
    <select id="selInteressadosProcedimento" name="selInteressadosProcedimento" class="infraSelect" multiple="multiple" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>"> 
  ...
  <div id="divInteressadoUsuario" class="infraAreaDados" style="height:5em;">    
    <label id="lblInteressadoUsuario" for="txtInteressadoUsuario" accesskey="I" class="infraLabelObrigatorio"><span class="infraTeclaAtalho">I</span>nteressado:</label>
    <input type="text" id="txtInteressadoUsuario" name="txtInteressadoUsuario" class="infraText" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>"/>
    <input type="hidden" id="hdnIdInteressadoUsuario" name="hdnIdInteressadoUsuario" class="infraText" value="" />
  </div>
  ...
  <input type="hidden" id="hdnIdTipoProcedimento" name="hdnIdTipoProcedimento" value="<?=$objProcedimentoDTO->getNumIdTipoProcedimento()?>" />
  <input type="hidden" id="hdnNomeTipoProcedimento" name="hdnNomeTipoProcedimento" value="<?=PaginaSEI::tratarHTML($objProcedimentoDTO->getStrNomeTipoProcedimento())?>" />
  <input type="hidden" id="hdnAssuntos" name="hdnAssuntos" value="<?=$_POST['hdnAssuntos']?>" />
  <input type="hidden" id="hdnInteressadosProcedimento" name="hdnInteressadosProcedimento" value="<?=PaginaSEI::tratarHTML($_POST['hdnInteressadosProcedimento'])?>" />
  <input type="hidden" id="hdnIdProcedimento" name="hdnIdProcedimento" value="<?=$objProcedimentoDTO->getDblIdProcedimento();?>" />
  <input type="hidden" id="hdnProtocoloFormatado" name="hdnProtocoloFormatado" value="<?=$objProtocoloDTO->getStrProtocoloFormatado();?>" />
  <input type="hidden" id="hdnStaNivelAcessoGlobal" name="hdnStaNivelAcessoGlobal" value="<?=$strStaNivelAcessoGlobal?>" />
