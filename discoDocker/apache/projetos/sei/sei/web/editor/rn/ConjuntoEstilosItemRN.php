...
class ConjuntoEstilosItemRN extends InfraRN {
  ...
  protected function consultarConectado(ConjuntoEstilosItemDTO $objConjuntoEstilosItemDTO){
    try {
      ...
      if ($ret != null){
        if ($objConjuntoEstilosItemDTO->isRetStrFormatacao()){
          while (strpos($ret->getStrFormatacao(), '\'\'') !== false) {
            $ret->setStrFormatacao(str_replace('\'\'', '\'', $ret->getStrFormatacao()));
          }
        }
      }
  ...
  protected function listarConectado(ConjuntoEstilosItemDTO $objConjuntoEstilosItemDTO) {
    try {
      ...
      if ($objConjuntoEstilosItemDTO->isRetStrFormatacao()){
        foreach($ret as $item) {
          while (strpos($item->getStrFormatacao(), '\'\'') !== false) {
            $item->setStrFormatacao(str_replace('\'\'', '\'', $item->getStrFormatacao()));
          }
        }
      }
