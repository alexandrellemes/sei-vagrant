...
try {
  ...
  foreach($_POST as $item){
    if (is_array($item)){
      //unset($_POST['pwdSenha']);
      //LogSip::getInstance()->gravar('Dados de login inválidos: '. print_r($_POST, true));
      die('Dados inválidos.');
    }
  }

  //filtra parametros que serao utilizados no action
  $strParametrosAction = '';
  ...
  $strCaptchaPesquisa = $_SESSION['SIP_CAPTCHA'];
  $strCodigoParaGeracaoCaptcha = InfraCaptcha::obterCodigo();
  $_SESSION['SIP_CAPTCHA'] = hash('SHA512',InfraCaptcha::gerar($strCodigoParaGeracaoCaptcha));
  ...
  //Se fez POST
	if (isset($_POST['sbmLogin'])){
      ...
      if ($_SESSION['SIP_NUM_FALHA_LOGIN'] >= $numLoginSemCaptcha && hash('SHA512',$_POST['txtCaptcha']) != $strCaptchaPesquisa) {
...
<form name="frmLogin" action="login.php?<?=$strParametrosAction?>" method="post" onsubmit="return validarForm();">
