...
try {
  ...
  if ($numRegistros >0){    
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td valign="top">'.PaginaSEI::getInstance()->getTrCheck($i,$arrObjContatoDTO[$i]->getNumIdContato(),$arrObjContatoDTO[$i]->getStrNome()).'</td>';
      }
      ...
      if ($bolAcaoAlterarTemporario){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=contato_alterar_temporario&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_tipo_contato='.$numIdTipoContato.'&id_contato='.$arrObjContatoDTO[$i]->getNumIdContato()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="imagens/alterar.gif" title="Alterar Contato Temporário" alt="Alterar Contato Temporário" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoDesativarContexto){
        $strResultado .= '<a href="#ID-'.$arrObjContatoDTO[$i]->getNumIdContato().'"  onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="imagens/desativar.gif" title="Desativar Contato" alt="Desativar Contato" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoExcluirContexto){
        $strResultado .= '<a href="#ID-'.$arrObjContatoDTO[$i]->getNumIdContato().'"  onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="imagens/excluir.gif" title="Excluir Contato" alt="Excluir Contato" class="infraImg" /></a>&nbsp;';
      }
...
<form id="frmContatoRelatorioTemporarios" method="post" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  PaginaSEI::getInstance()->abrirAreaDados('10em');
	?>
  <label id="lblTextoPesquisaContatos" class="infraLabel" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">Texto para Pesquisa:</label>
  ...
  <label id="lblContatoSubstituicao" class="infraLabel" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">Contato para Substituição:</label>
