...
class SolrUtil {
  ...
  public static function formatarCaracteresEspeciais($q){
    ...
    if (ConfiguracaoSEI::getInstance()->getValor('Solr','Servidor')==null) {
      $objInfraPagina->setStrMensagem(SolrUtil::$MSG_ERRO_SERVIDOR_NAO_CONFIGURADO, InfraPagina::$TIPO_MSG_AVISO);
    }else if ($e instanceof InfraException && $e->contemValidacoes()){
      $objInfraPagina->setStrMensagem($e->__toString(), InfraPagina::$TIPO_MSG_AVISO);
      LogSEI::getInstance()->gravar(InfraException::inspecionar($e));
    }else {
      $objInfraPagina->setStrMensagem(SolrUtil::$MSG_ERRO_PESQUISA, InfraPagina::$TIPO_MSG_AVISO);
      LogSEI::getInstance()->gravar(InfraException::inspecionar($e));
    }
  }
}
