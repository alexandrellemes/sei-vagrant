...
try {
  ...
  switch ($_GET['acao']) {
    case 'infra_log_excluir':
      ...
      header('Location: ' . SessaoInfra::getInstance()->assinarLink('controlador.php?acao=' . $_GET['acao_origem'] . $strParametros));
      die;
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_log_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_infra_log='.$arrObjInfraLogDTO[$i]->getNumIdInfraLog()).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/consultar.gif" title="Consultar Log" alt="Consultar Log" class="infraImg" /></a> ';
      }

      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_log_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_infra_log='.$arrObjInfraLogDTO[$i]->getNumIdInfraLog()).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Log" alt="Alterar Log" class="infraImg" /></a> ';
      }
      ...
      if ($bolAcaoExcluir){
        $strResultado .= '<a onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Log" alt="Excluir Log" class="infraImg" /></a> ';
      }
...
PaginaInfra::getInstance()->montarTitle(PaginaInfra::getInstance()->getStrNomeSistema().' - '.$strTitulo.' ('.$objInfraLogDTO->getNumTotalRegistros().')');
PaginaInfra::getInstance()->montarStyle();
PaginaInfra::getInstance()->abrirStyle();
?>

  #lblTipos {position:absolute;left:0%;top:5%;}
  #selTipos, .multipleSelect  {position:absolute;left:11%;top:0%;width:16.5%;}
  ...
  #lblDthFinal {position:absolute;left:31.5%;top:25%;}
  ...
  #divSinAutoAtualizar {position:absolute;left:11%;top:80%;}
...
if (PaginaInfra::getInstance() instanceof InfraPaginaEsquema){
  PaginaInfra::getInstance()->abrirBody($strTitulo,'onload="inicializar();"');
}else{
  PaginaInfra::getInstance()->abrirBody('onload="inicializar();"');
}
?>
  <form id="frmInfraLogLista" method="post" onsubmit="return validarForm();" action="<?=SessaoInfra::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'].$strParametros)?>">
    ...
    PaginaInfra::getInstance()->abrirAreaDados('13em');
    ?>

    <?if ($bolTipos) {?>
      <label id="lblTipos" class="infraLabelObrigatorio">Tipo:</label>
      <select multiple id="selTipos" name="selTipos[]" class="infraSelect multipleSelect" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>">
        <?=$strOptionsTipo?>
      </select>
    <?}?>
    ...
    <img src="<?=PaginaInfra::getInstance()->getDiretorioImagensGlobal()?>/calendario.gif" id="imgCalDthInicial" title="Selecionar Data/Hora Inicial" alt="Selecionar Data/Hora Inicial" class="infraImg" onclick="infraCalendario('txtDthInicialLog',this,true,'<?=InfraData::getStrDataAtual().' 00:00'?>');" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />
    ...
    <img src="<?=PaginaInfra::getInstance()->getDiretorioImagensGlobal()?>/calendario.gif" id="imgCalDthFinal" title="Selecionar Data/Hora Final" alt="Selecionar Data/Hora Final" class="infraImg" onclick="infraCalendario('txtDthFinalLog',this,true,'<?=InfraData::getStrDataAtual().' 23:59'?>');" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />
