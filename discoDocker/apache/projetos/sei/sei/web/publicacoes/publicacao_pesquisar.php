  ...
  $objResultadoPesquisaSolrDTO = null;
	...
	switch($_GET['acao']){
      ...
      if (isset($_POST['rdoDataPublicacao'])) {
        ...
        $objPesquisaPublicacaoSolrDTO->setStrStaTipoData($_GET['rdo_data_publicacao']);
        $bolParametros = true;
      }else{
        $objPesquisaPublicacaoSolrDTO->setStrStaTipoData('I');
      }
      ...
      $numRegistros = 0;
	    ...
	    if (count($_POST) || $bolParametros){
          ...
          SolrUtil::tratarErroPesquisa(PaginaPublicacoes::getInstance(), $e);
				}
	    }
...
#txtDataDocumento {position:absolute;left:20%;top:72%;width:10%;}
#imgDataDocumento {position:absolute;left:31%;top:74%;}
...
<div id="divPrincipal" class="infraAreaDados" style="height:30em;">
  ...
  <label id="lblInteiroTeor" for="txtInteiroTeor" accesskey="" class="infraLabelOpcional">Inteiro Teor:</label>
	<input type="text" id="txtInteiroTeor" name="txtInteiroTeor" class="infraText" value="<?=str_replace('\\','',str_replace('"','&quot;',$objPesquisaPublicacaoSolrDTO->getStrPalavrasChave()))?>" maxlength="100" tabindex="<?=PaginaPublicacoes::getInstance()->getProxTabDados()?>" />
  <a id="ancAjuda" href="<?=$strLinkAjuda?>" target="_blank" title="Ajuda para Pesquisa" tabindex="<?=PaginaPublicacoes::getInstance()->getProxTabDados()?>"><img src="<?=PaginaPublicacoes::getInstance()->getDiretorioImagensGlobal()?>/ajuda.gif" class="infraImg"/></a>
	...
	<input type="text" id="txtResumo" name="txtResumo" class="infraText" value="<?=$objPesquisaPublicacaoSolrDTO->getStrResumo()?>" onkeypress="return infraMascaraTexto(this,event,50);" maxlength="100" tabindex="<?=PaginaPublicacoes::getInstance()->getProxTabDados()?>" />
  ...
  <input type="text" id="txtNumero" name="txtNumero" class="infraText" value="<?=$objPesquisaPublicacaoSolrDTO->getStrNumero()?>" onkeypress="return infraMascaraTexto(this,event,50);" maxlength="15" tabindex="<?=PaginaPublicacoes::getInstance()->getProxTabDados()?>" />

  <label id="lblProtocoloPesquisa" for="txtProtocoloPesquisa" accesskey="" class="infraLabelOpcional">Protocolo:</label> 
  <input type="text" id="txtProtocoloPesquisa" name="txtProtocoloPesquisa" class="infraText" value="<?=$objPesquisaPublicacaoSolrDTO->getStrProtocoloPesquisa()?>" tabindex="<?=PaginaPublicacoes::getInstance()->getProxTabDados()?>" />
  ...
  <label id="lblDataPublicacao" class="infraLabelObrigatorio">Data de Publicação:</label>
	<input type="radio" id="optHoje" name="rdoDataPublicacao" value="H" onclick="tratarPeriodo();" <?=($objPesquisaPublicacaoSolrDTO->getStrStaTipoData()=='H'  ? 'checked="checked"':'')?> class="infraRadio"/>
  ...
  <input type="radio" id="optIndeterminada" name="rdoDataPublicacao" value="I" onclick="tratarPeriodo();" <?=($objPesquisaPublicacaoSolrDTO->getStrStaTipoData()=='I' || $objPesquisaPublicacaoSolrDTO->getStrStaTipoData()==null ? 'checked="checked"':'')?> class="infraRadio"/>
  ...
  <input type="radio" id="optPeriodoExplicito" name="rdoDataPublicacao" value="E" onclick="tratarPeriodo();" <?=($objPesquisaPublicacaoSolrDTO->getStrStaTipoData()=='E' ? 'checked="checked"':'')?> class="infraRadio" />
