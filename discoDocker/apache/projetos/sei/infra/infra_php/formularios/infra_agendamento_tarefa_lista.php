...
try {
  ...
  PaginaInfra::getInstance()->salvarCamposPost(array('txtComandoListar', 'txtComplementoListar', 'selSinAtivoListar', 'selSinSucessoListar', 'selStaPeriodicidadeExecucaoListar'));
  ...
  if(!empty($strComplemento)){
    $objInfraAgendamentoTarefaDTO->setStrPeriodicidadeComplemento('%'.$strComplemento.'%', InfraDTO::$OPER_LIKE);
  }
  ...
  */
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if(!empty($strUltimaExecucao)){
        if(!InfraString::isBolVazia($arrObjInfraAgendamentoTarefaDTO[$i]->getDthUltimaConclusao()) &&
            InfraData::compararDataHorasSimples($arrObjInfraAgendamentoTarefaDTO[$i]->getDthUltimaExecucao(), $arrObjInfraAgendamentoTarefaDTO[$i]->getDthUltimaConclusao()) >= 0){
          $numTimestampUltimaExecucao = InfraData::compararDataHora($arrObjInfraAgendamentoTarefaDTO[$i]->getDthUltimaExecucao(), $arrObjInfraAgendamentoTarefaDTO[$i]->getDthUltimaConclusao());
          $strDuracaoUltimaExecucao = $numTimestampUltimaExecucao == 0? '0s' : InfraData::formatarTimestamp($numTimestampUltimaExecucao);
      ...
      $strInformacoesComplementares .= '* DESCRIÇÃO: '.trim($arrObjInfraAgendamentoTarefaDTO[$i]->getStrDescricao())."<br /><br />";
      $strInformacoesComplementares .= '* PARÂMETROS: '.trim(str_replace(',',', ',$arrObjInfraAgendamentoTarefaDTO[$i]->getStrParametro()))."<br /><br />";
      $strInformacoesComplementares .= '* COMUNICAÇÃO DE ERROS: '.trim($arrObjInfraAgendamentoTarefaDTO[$i]->getStrEmailErro());

      $strResultado .= '<img  id="imgAjudaBeneficio" src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/ajuda.gif" alt="" onmouseover="return infraTooltipMostrar(\''.PaginaInfra::getInstance()->formatarParametrosJavaScript($strInformacoesComplementares).'\', \'Informações Complementares\', \'550\');" onmouseout="return infraTooltipOcultar();" class="infraImgNormal" />&nbsp;';
  ...
  $strItensSelStaPeriodicidadeExecucao = InfraAgendamentoTarefaINT::montarSelectStaPeriodicidadeExecucao('null','Todas',$strStaPeriodicidadeExecucao);
...
<form id="frmInfraAgendamentoTarefaLista" method="post" action="<?=SessaoInfra::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  <?
  PaginaInfra::getInstance()->abrirAreaDados('4em');
  ?>

  <label id="lblComandoListar" for="txtComandoListar" class="infraLabelOpcional">Comando:</label>
  <input type="text" id="txtComandoListar" name="txtComandoListar" maxlength="100" class="infraText" value="<?=$strComando?>" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />

  <label id="lblComplementoListar" for="txtComplementoListar" class="infraLabelOpcional">Complemento:</label>
  <input type="text" id="txtComplementoListar" name="txtComplementoListar" maxlength="100" class="infraText" value="<?=$strComplemento?>" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />

  <label id="lblSinSucessoListar" for="selSinSucessoListar" accesskey="" class="infraLabelOpcional">Sucesso:</label>
  <select id="selSinSucessoListar" name="selSinSucessoListar" class="infraSelect" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" >
    <?=$strItensSelSinSucesso?>
  </select>

  <label id="lblSinAtivoListar" for="selSinAtivoListar" accesskey="" class="infraLabelOpcional">Ativo:</label>
  <select id="selSinAtivoListar" name="selSinAtivoListar" class="infraSelect" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" >
    <?=$strItensSelSinAtivo?>
  </select>

  <label id="lblStaPeriodicidadeExecucaoListar" for="selStaPeriodicidadeExecucaoListar" accesskey="" class="infraLabelOpcional">Periodicidade:</label>
  <select id="selStaPeriodicidadeExecucaoListar" name="selStaPeriodicidadeExecucaoListar" class="infraSelect" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" >
