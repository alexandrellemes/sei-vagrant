...
//LEITURA		
    public function converterStr($tabela,$campo){
      ...
      return $ret;
		...
		public function formatarPesquisaStr($strTabela,$strCampo,$strValor,$strOperador,$bolCaseInsensitive){
			if ($bolCaseInsensitive){
			  return 'uppercase(varchar('.$strCampo.')) '.$strOperador.' \''.str_replace('\'','\'\'',InfraString::transformarCaixaAlta($strValor)).'\' ';	
			}else{
	  ...
	  public function abrirConexao() {
	     try{
				...
				if (InfraDebug::isBolProcessar()) {
					InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->abrirConexao] ' . $this->getIdBanco());
				}
  	    ...
  	    if ($this->numTipoInstalacao == 1) {
  	      $this->conexao = ingres_connect($string_conexao,$this->getUsuario(),$this->getSenha(),$this->getOpcoes());
  	    }else if ($this->numTipoInstalacao == 2) {
  	      $this->conexao = ingres2_connect($string_conexao,$this->getUsuario(),$this->getSenha(),$this->getOpcoes());
  	    }else if ($this->numTipoInstalacao == 3) {
  	      $this->conexao = ingres_connect($string_conexao,$this->getUsuario(),$this->getSenha(),$this->getOpcoes());
  	    }
	  ...
	  public function fecharConexao() {
			...
			if (InfraDebug::isBolProcessar()) {
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->fecharConexao] ' . $this->getIdConexao());
			}
    ...
    public function abrirTransacao(){
			...
			if (InfraDebug::isBolProcessar()) {
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->abrirTransacao] ' . $this->getIdConexao());
			}
	  ...
	  public function confirmarTransacao() {
			...
			if (InfraDebug::isBolProcessar()) {
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->confirmarTransacao] ' . $this->getIdConexao());
			}
	  ...
	  public function cancelarTransacao() {
			...
			if (InfraDebug::isBolProcessar()) {
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->cancelarTransacao] ' . $this->getIdConexao());
			}
	  ...
	  public function consultarSql($sql, $arrCamposBind = null) {
			...
			if (InfraDebug::isBolProcessar()) {
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->consultarSql] ' . $sql);
	  	...
	  	if ($this->numTipoInstalacao == 1) {
	  	  ...
	  	  if (ingres2_errno()){
	  	    throw new InfraException(ingres2_errno().': '.ingres2_error(),null,substr($sql,0,INFRA_TAM_MAX_LOG_SQL));
	  	  }
	  	  ...
	  	  if (ingres_errno()){
	  	    throw new InfraException(ingres_errno().': '.ingres_error(),null,substr($sql,0,INFRA_TAM_MAX_LOG_SQL));
	  	  }
        ...
        while ($linha = ingres_fetch_array($resultado, INGRES_ASSOC)) {
          ...
          //InfraDebug::getInstance()->gravarInfra('[InfraIngres('.$this->numTipoInstalacao.')->consultarSql] '.$i);
			...
			if (InfraDebug::isBolProcessar()) {
				...
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->consultarSql] ' . $numSeg . ' s');
			}
	  ...
	  public function paginarSql($sql,$ini,$qtd) {
			...
			if (InfraDebug::isBolProcessar()) {
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->paginarSql] ' . $sql);
			}
	  ...
	  public function executarSql($sql, $arrCamposBind = null) {
			...
			if (InfraDebug::isBolProcessar()) {
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->executarSql] ' . substr($sql, 0, INFRA_TAM_MAX_LOG_SQL));
				$numSeg = InfraUtil::verificarTempoProcessamento();
			}
	    ...
	    //InfraDebug::getInstance()->gravarInfra('[InfraIngres('.$this->numTipoInstalacao.')->executarSql] 40');
			...
			if (InfraDebug::isBolProcessar()) {
				$numSeg = InfraUtil::verificarTempoProcessamento($numSeg);
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->executarSql] ' . $numReg . ' registro(s) afetado(s)');
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->executarSql] ' . $numSeg . ' s');
			}
	  ...
	  public function prepararSql($sql){
			...
			if (InfraDebug::isBolProcessar()) {
				InfraDebug::getInstance()->gravarInfra('[InfraIngres(' . $this->numTipoInstalacao . ')->prepararSql] ' . $sql);
			}
