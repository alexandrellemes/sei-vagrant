...
require_once dirname(__FILE__).'/../../../../../../lib/Diff/Renderer/Text/Onlydiff.php';
...
final class AntiXSS
{
  ...
  private $_xss_diff_processing = true;
   ...
   * @return string
   */
  public function getXssDiff(){
    return $this->_xss_diff;
  }

  public function setXssDiffProcessing($bolXssDiffProcessing){
    return $this->_xss_diff_processing = $bolXssDiffProcessing;
  }
  ...
  private function _do($str)
  {
    ...
    if ($this->xss_found !== true) {
      ...
      if($this->xss_found === true && $this->_xss_diff_processing === true) {
        $diff = new \Diff(explode("\n",$str_backup), explode("\n",$str), array('ignoreWhitespace' => true,'ignoreCase' => true));
        $renderer = new \Diff_Renderer_Text_Onlydiff();
        $this->_xss_diff .=$diff->render($renderer);
      }
    }
  ...
  private function _do_never_allowed($str)
  {
    ...
    //executar regex individualmente e só tratar base64 se houver string base64 no conteúdo
    foreach (self::$_never_allowed_regex as $regex) {
      if(strpos($regex,'base64')===false || strpos($str,'base64')!==false) {
  ...
  private function _do_never_allowed_afterwards($str)
  {
    ...
    if (null === $NEVER_ALLOWED_STR_AFTERWARDS_CACHE) {
      ...
      $NEVER_ALLOWED_STR_AFTERWARDS_CACHE = self::$_never_allowed_str_afterwards;
    }

    foreach ($NEVER_ALLOWED_STR_AFTERWARDS_CACHE as $neverAllowedStr) {
      $str = preg_replace('#' . $neverAllowedStr . '#isU', $this->_replacement, $str);
    }
    return (string)$str;
  }
   ...
   * @return string
   */
  private function _js_removal_callback(array $match, $search)
  {
    if (!$match[0]) {
      return '';
    }
  ...
  {
    if(is_array($this->_evil_html_tags)){
      foreach ($this->_evil_html_tags as $evil_tag) {
        $str = \preg_replace_callback(
            '#<(/*\s*)(' . $evil_tag . ')((?![a-zA-Z0-9])[^><]*)([><]*)#i',
            array(
                $this,
