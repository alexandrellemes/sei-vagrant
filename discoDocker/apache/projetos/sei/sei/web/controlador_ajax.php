  ...
  switch($_GET['acao_ajax']){
   ...
   case 'uf_montar_select_sigla':
    ...
    case 'assinatura_verificar_confirmacao':
      ...
      $numAssinaturasPendentes = $objAssinaturaRN->contarRN1324($objAssinaturaDTO);

      $xml = InfraAjax::gerarXMLComplementosArray(array('assinatura_confirmada' => ($numAssinaturasPendentes==0?'S':'N')));
      break;
   ...
   case 'unidade_auto_completar_envio_processo':
      $arrObjUnidadeDTO = UnidadeINT::autoCompletarEnvioProcesso($_POST['palavras_pesquisa']);
      $xml = InfraAjax::gerarXMLItensArrInfraDTO($arrObjUnidadeDTO,'IdUnidade', 'Sigla');
      break;
   ...
   case 'lembrete_atualizar':
     ...
     $objLembreteDTO->setNumIdLembrete($_POST['id']-1000);
     if ($_POST['oper']!='disable'){
       ...
       $objLembreteDTO->setStrConteudo(utf8_decode(urldecode($_POST['description'])));
     ...
     $resultado=LembreteINT::atualizarLembrete($_POST['oper'],$objLembreteDTO);
     $xml=InfraAjax::gerarXMLComplementosArray(array("resultado"=>$resultado));
     break;
   ...
   case 'contexto_carregar_nome':
      $strOptions = ContextoINT::montarSelectNome('null',' ',null,$_POST['id_orgao']);
      $xml = InfraAjax::gerarXMLSelect($strOptions);
      break;
